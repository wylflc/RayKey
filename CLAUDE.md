...
| 作业 | 实测峰值 | 实测耗时 | 规矩 |
| --- | ---: | ---: | --- |
| 单个回测进程（`backtest_valuation_strategy.py`，23 起点中的一个） | **1.25 GB**（全市场或面板子集状态文件都一样，内存由宇宙内代码决定） | 读全市场 1.9 GB 状态文件 ~26 s；读面板子集文件 ~6 s | — |
| 扫描器（`sweep_backtest_configs.py`） | 并发数 × 1.25 GB | 单臂 23 起点：面板子集文件 ~1.5 分钟、全市场文件 ~5 分钟（2 并发时读文件相互争用）；**与建带/物化链并发且 swap 已用 ≥3.5 GB 时实测放慢到 ~60 分钟（2026-08-23）**，长作业仍应串行 | **`--workers` 缺省 2**；上调前先算 `并发×1.25GB + 其它作业 ≤ 5GB` |
...
1. **并发按实测峰值加总，总预算 5 GB**（给系统与用户自己的程序留 3 GB）。已实跑无事的组合：
   建带 `--all`（2.3 GB）＋ 1 个回测进程（1.25 GB）＝ 3.6 GB；2 并发扫描（2.5 GB）＋ 若干只读大文件的轻脚本。
   **不得**：两个建带；2 并发扫描 ＋ 建带（4.8 GB 贴线）；任何 `--workers ≥ 3` 的扫描叠加别的重作业。
