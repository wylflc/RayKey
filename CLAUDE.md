...
| 作业 | 单进程峰值内存 | 申请 |
| --- | ---: | --- |
| 单个回测进程（`backtest_valuation_strategy.py`，23 起点中的一个） | 1.49 GB（读候选侧＋持仓侧两份状态文件；只读一份时 1.25 GB） | `-n 16` |
| 扫描器（`sweep_backtest_configs.py --workers N`） | N × 1.49 GB | `-n N`，N 取 16/32/48…（`-n 32` 给 56 GB，够 32 并发的 47.7 GB） |
| 全市场建带（`build_historical_valuation_bands.py --all`） | 2.3 GB | `-n 16` |
| 银行股利折现覆盖（`rebuild_bank_bands.py`） | 0.06 GB | `-n 16` |
| 六步链其余各步 | ≤ 建带 | `-n 16` |

峰值内存由代码决定、跨机器不变，可直接照用；**耗时一律以本机重测为准**（GPFS 的 I/O 与本地 SSD 不同）——
先小样本跑一次，`sacct -j <id> --format=Elapsed,MaxRSS` 读实际值，再定 `-t` 与并发数。
