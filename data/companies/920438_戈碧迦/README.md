...
**2026-08-23 用户点名建档**（§6.1：点名公司一律建档给带；结论 boundary_pending，不入池不进交易层）
...
**手写：可证伪命题与方法分歧**（不作为带；带与 `P/V` 以上段为准）：boundary_pending（回报不足、规模小，不入池）：可证伪命题——特种玻璃能否带来持续高于 15% 的 ROE。
