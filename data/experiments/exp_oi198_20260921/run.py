"""OI-198: OLD (pre-fix snapshot inputs) vs NEW (fixed production inputs), same engine and BASE. 14 starts, full/A/U, 0bp."""
import csv
import hashlib
import json
import os
...
PROD_INPUTS = ['scripts/backtest_valuation_strategy.py', 'scripts/corporate_actions.py', 'scripts/sweep_backtest_configs.py',
               ...
               'data/raw/corporate_actions/a_share_corporate_actions.csv', 'data/reference/a_share_exright_terms.csv',
               'data/reference/a_share_action_component_corrections.csv', 'scripts/build_historical_valuation_bands.py']
...
def prepare():
    ...
    inputs |= {str(p.relative_to(ROOT)): digest(p) for p in [PANEL, EXP/'legacy_bridge_keys.json', EXP/'classification.json', EXP/'census.json']}
    for arm in ('OLD','NEW'):
    ...
    frozen = json.loads((EXP/'before_manifest.json').read_text())
    for name, expected in frozen['raw_inputs'].items():
        assert digest(ROOT/name) == expected == digest(EXP/'old_inputs'/name), name
...
def one(job):
    ...
    command += ['--universe-file', str(PANEL), '--daily-states', str(EXP/'states'/arm/'a_share_daily_states_adopted.csv'),
                ...
                '--equity-bond-data', str(OLD/'data/reference/equity_bond_csi300.csv')]
    env['EXP_ARM'] = arm
    env['EXP_ACTIONS_FILE'] = str(OLD/'data/raw/corporate_actions/a_share_corporate_actions.csv')
    if excluded:
...
if __name__ == '__main__':
    ...
    with (OLD / 'data/backtest' / f'summary_BASE{sw.EX5_ANCHOR_START.replace("-", "")}.csv').open() as f:
