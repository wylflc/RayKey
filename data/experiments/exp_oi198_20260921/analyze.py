"""Pair OLD/NEW per start and report the §12.1 Track A guardrail (OI-198, minority zero-floor synchronization)."""
import contextlib
import csv
import json
...
def main():
    ...
    for group in groups[1:]:
        ...
        with (EXP / f'report_full_{group}.txt').open('w') as f, contextlib.redirect_stdout(f):
            sw.report(combined, 'OI-198 少数股东负账面零下界同步：NEW（修复后）vs OLD（85177907），同 BASE，0bp（轨道 A 见 verification.json）')
