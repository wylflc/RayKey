...
def main():
    ...
    for name in FILES:
        ...
        assert old.keys()==rebuild.keys(), name
        assert new.keys() <= old.keys(), (name, 'unexpected new daily keys')
        diffs=[]; changed=Counter(); changed_codes=Counter(); frozen_diff=Counter(); records=[]
        retained_rounding=[]
        for key in sorted(old):
            o,b=old[key],rebuild[key]
            n=new.get(key, {})
            ...
            if n: records.append({k:n.get(k,'') for k in o})
