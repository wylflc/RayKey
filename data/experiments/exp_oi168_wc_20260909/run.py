"""Run the OI-168 working-capital controls and dose comparisons, full/A/U."""
import argparse
import csv
...
def scan(config,group,workers,exclude=None):
    command=[sys.executable,str(EXP/"scan.py"),str(EXP/config),
             ...
             "--workers",str(workers),"--title","OI-168营运资金：去重与经营融资应收修复"]
...
def main():
    ...
    previous=ROOT/"data/experiments/exp_maintenance_cash_20260909/summary_rows.csv"
    old={}
    with previous.open() as f:
        for row in csv.DictReader(f):
            if row["arm"] in ("BASE","WCREPSEP09") and row["group"] in ("full","A"):
                old[(row["group"],row["arm"],row["start"])]=row
    checks=[];diagnostics=[]
    for group in ("full","A"):
        for arm in ("BASE","WCREPSEP09"):
            for start,row in groups[group][arm].items():
                prev=old[(group,arm,start)]
                for field in (*sw.FIELDS,"前五赢家","首个净值日","末次净值日"):
                    assert row[field]==prev[field],(group,arm,start,field,row[field],prev[field])
                checks.append(f"{group}:{arm}:{start}:all_report_fields_identical")
    ...
    save("verification.json",{"completed_beijing":datetime.now(ZoneInfo("Asia/Shanghai")).isoformat(),
        ...
        "candidate_models":1,"candidate_combinations":5,"metric":sw.METRIC_VERSION})
    assert unchanged
