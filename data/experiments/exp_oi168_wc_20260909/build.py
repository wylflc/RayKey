"""Build OI-168 working-capital controls and verify the frozen original baseline."""
import bisect
...
MODES = {"BASE": "legacy", "WCREP": "reported", "WCOP": "operating"}
...
def main():
    ...
    previous=json.loads((EXP.parent/"exp_maintenance_cash_20260909/manifest.json").read_text())["inputs"]
    for path, digest in previous.items():
        if path.startswith(("data/raw/", "data/reference/", "data/processed/", "data/interim/")):
            assert before[path]==digest, f"Input epoch changed: {path}"
    ...
    save("manifest.json", {"started_beijing":datetime.now(ZoneInfo("Asia/Shanghai")).isoformat(),
        ...
        "modes":MODES,"candidate_combinations":5,
    ...
    for tag,mode in MODES.items():
        ...
        for side in ("base","b2"):
            ...
            command=common+["--maintenance-weight","0","--wc-aggregation",mode,"--out-bands",str(band),"--out-daily",str(raw)]
        ...
        if tag=="BASE":config.append(f"BASE|{states}")
            ...
            config.append(f"WCOPRAWSEP09|{states} --width {1-base_line:.4f} --swap-margin 0.15")
