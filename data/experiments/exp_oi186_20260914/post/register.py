"""Register verified confirmed-fill BASE summaries, preserving previous ledger rows."""
import csv
import hashlib
import json
import os
...
def main():
    ...
    assert v['track_a_all_costs_pass'] and v['complete_pairing'] and v['zero_negative_cash']
    assert v['current_baseline_winners_unchanged'], '须另外补齐新 BASE 前五赢家剔除集'
    winners=json.loads((EXP/'winners.json').read_text())['by_arm']['CONF']
    rows=[r for r in read(EXP/'summary_rows.csv') if r['arm']=='CONF' and r['group'] in ('full','A') and r['bp']=='0']
    assert len(rows)==28
    ...
    for r in rows:
        ...
        tmp=target.with_name('.'+target.name+'.oi186')
    ...
    (EXP/'registration.json').write_text(json.dumps(dict(registered_base_rows=28,old_rows_preserved=True,
        new_keys=len(after)-len(before),batch_new_keys=len(after)-len(committed),final_job='26680547',strategy_marker='lrc3',entry_point='clean_derived_artifacts.write_ledger',published=published),indent=2)+'\n')
    print('REGISTERED 28 confirmed-fill BASE summaries; old rows preserved.')
