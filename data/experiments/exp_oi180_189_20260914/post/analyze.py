...
def main():
    ...
    for bp in (0,10,20,30):
        for group in ('full','A','U'):
            ...
            arms={a:{r['start']:numeric(r) for r in selected if r['arm']==a} for a in ('BASE','SIGNAL')}
            defects=sw.pairing_defects(arms,'SIGNAL','BASE',sw.DEFAULT_STARTS,f'{group}:{bp}')
            assert not defects,defects
            nav_equal=0
            for start in sw.DEFAULT_STARTS:
                n,b=arms['SIGNAL'][start],arms['BASE'][start]
                ...
                pairs.append(p)
                pair=[next(r for r in selected if r['arm']==a and r['start']==start) for a in ('BASE','SIGNAL')]
        ...
        for group in ('A', 'U') if winners['U']!=winners['A'] else ('A',):
            ...
            with (EXP/f'report_full_{group}_{bp}.txt').open('w') as f,contextlib.redirect_stdout(f):
                sw.report(combined, f'OI-180/181/184 信号日执行一致性修复 vs 旧执行实现，{bp}bp（轨道A见 verification.json）')
    ...
    cost_pairs=[]
    for group in ('full','A'):
        base0={r['start']:numeric(r) for r in rows if r['arm']=='BASE' and r['group']==group and int(r['bp'])==0}
        for bp in (10,20,30):
            stressed={r['start']:numeric(r) for r in rows if r['arm']=='BASE' and r['group']==group and int(r['bp'])==bp}
            assert set(stressed)==set(base0)==set(sw.DEFAULT_STARTS)
            current=[dict(start=d,P_pp=100*sw.start_delta(stressed[d],base0[d],sw.WIN5_KEY),
                          CAGR_pp=100*sw.start_delta(stressed[d],base0[d],'年化'),
                          DD5_pp=100*sw.start_delta(stressed[d],base0[d],'滚动5年回撤中位')) for d in sw.DEFAULT_STARTS]
            cost_pairs.append(dict(group=group,bp=bp,**{k:st.median(r[k] for r in current) for k in ('P_pp','CAGR_pp','DD5_pp')}))
    with (EXP/'base_cost_vs_zero.csv').open('w',newline='') as f:
        w=csv.DictWriter(f,fieldnames=list(cost_pairs[0]));w.writeheader();w.writerows(cost_pairs)
    save('verification.json',dict(complete_pairing=True,zero_negative_cash=all(r['负现金日数']=='0' for r in rows),
                                 ...
                                 current_baseline_winners_unchanged=winners['by_arm']['BASE']==winners['by_arm']['SIGNAL'],
