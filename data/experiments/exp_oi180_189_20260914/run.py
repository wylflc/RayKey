"""Frozen-input full/A/U and execution-cost comparisons for OI-180/181/184."""
import csv
import hashlib
import json
import os
...
def one(job):
    ...
    command=[sys.executable,str(EXP/'engine.py'),*shlex.split(sw.BASE),*manifest['state_args'],
             '--since',start,'--execution-consistency','legacy' if arm=='BASE' else 'signal',
...
def batch(group,bp,excluded):
    jobs=[(arm,start,group,bp,excluded) for arm in ('BASE','SIGNAL') for start in sw.DEFAULT_STARTS]
...
if __name__=='__main__':
    ...
    rows=batch('full',0,[])
    winners={a:sorted(next(r for r in rows if r['arm']==a and r['start']==sw.EX5_ANCHOR_START)['前五赢家'].split('/')) for a in ('BASE','SIGNAL')}
    A=winners['BASE'];U=sorted(set(A)|set(winners['SIGNAL']))
