...
def main():
    ...
    for name in ('daily_entry_plan.csv','daily_sell_plan.csv'):
        checks[name]=len(read(ROOT/'data/processed'/name))
    ...
    for r in tracking:
        ...
        for key in ('current_shares','close','entry_stop_price'):
            assert r[key]==o[key],(r['security_code'],key,r[key],o[key])
    checks['holding_shares_close_anchor_unchanged']=True
    checks['tracking_changes']={r['security_code']:{k:[old_tracking[r['security_code']].get(k),v] for k,v in r.items()
        if old_tracking[r['security_code']].get(k)!=v} for r in tracking}
    ...
    assert events['002043']['2026-06-01'][0]==.312 and events['002043']['2026-06-01'].price[0]==.3080016
    assert events['603529']['2026-09-24'][0]==.392 and events['603529']['2026-09-24'].price[0]==.385
    assert events['300750']['2024-04-30'][0]==5.028
    assert events['600989']['2026-04-28'][0]==.4921
    checks['verified_price_terms']=len(actions.price_overrides())
    ...
    checks.update(candidates=len(candidates),action_components=len(components),original_shadow_snapshots_preserved=len(snapshots),
                  valuation_pool_rows=len(core),buy_sell_plan_rows={n:checks[n] for n in ('daily_entry_plan.csv','daily_sell_plan.csv')},
                  mindray={k:current['300760'][k] for k in ('close','model_intrinsic_value','model_pv','ma20','ma60')},
                  jobs=json.loads((EXP/'jobs.json').read_text()))
