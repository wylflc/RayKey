...
def main():
    ...
    for r in rows:
        ...
        tmp = target.with_name('.' + target.name + '.oi197')
    ...
    (EXP / 'registration.json').write_text(json.dumps(dict(registered_base_rows=28, overwritten_keys=overwritten, new_keys=len(after) - len(before),
        input_epoch='actual_vs_exchange_terms_20260921', precedent='OI-168 §12.217 同键重登', final_job=json.loads((EXP/'completed.json').read_text())['job_id'],
