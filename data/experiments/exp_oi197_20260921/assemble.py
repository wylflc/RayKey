...
def splice(source, block, dest, codes):
    ...
    for row in rows:
        if row['security_code'] in groups: groups[row['security_code']].append(row)
...
def main():
    ...
    for name in FILES:
        ...
        for key in sorted(old):
            ...
            # Two unchanged B2 rows retain their pre-existing last decimal.
            # This is an exact-key/value exception, never a numerical tolerance.
            if (name in ('roic_daily_raw_b2.csv','a_share_daily_states_b2.csv') and
                    key == ('300360','2016-09-27') and stored_diff ==
                    {k:['2.1440','2.1441'] for k in ('valuation_ratio','pv_equity')}):
                assert all(n[k] == b[k] for k in o), (name,key,'correction overlaps old rounding')
                retained_rounding.append(dict(key=key, differences=stored_diff))
                for k in stored_diff: n[k]=o[k]
                stored_diff={}
            if stored_diff:
                save('unexpected_old_rebuild_drift.json', dict(file=name, key=key, differences=stored_diff))
                raise AssertionError((name, key, stored_diff))
        ...
        evidence[name]=dict(changed_rows=len(diffs),changed_codes=changed_codes,changed_fields=changed,
                           old_rebuild_vs_stored=frozen_diff, retained_unchanged_rounding=retained_rounding,
                           splice=spliced, new_subset=digest(EXP/'states/NEW'/name))
