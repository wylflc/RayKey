...
def main():
    ...
    for name in FILES:
        ...
        tmp=target.with_name('.'+name+'.oi197')
    ...
    tmp=raw.with_name('.'+raw.name+'.oi197');shutil.copyfile(EXP/'actions_corrected.csv',tmp);os.replace(tmp,raw)
    terms = ROOT/'data/reference/a_share_exright_terms.csv'
    tmp = terms.with_name('.'+terms.name+'.oi197'); shutil.copyfile(EXP/'price_terms_corrected.csv',tmp); os.replace(tmp,terms)
    stamp(raw,'2026-09-21',inputs=[EXP/'source_manifest.json',EXP/'repairs.json',terms,
        ROOT/'data/reference/a_share_action_component_corrections.csv',ROOT/'data/reference/a_share_action_component_exclusions.csv'])
    ...
    correction='corrections/oi197_actions_20260921.json';migration='migrations/oi197_20260921.json'
    ...
    record=dict(issue='OI-197',created_at_beijing=shadow.sh.now(),before_code_sha256=before['code_sha256'],after_code_sha256=shadow.hashes(),
                before_manifest_sha256=digest(EXP/'shadow_manifest_before.json')['sha256'],
                validation='data/experiments/exp_oi197_20260921/shadow_validation.json', validation_sha256=digest(EXP/'shadow_validation.json')['sha256'],
