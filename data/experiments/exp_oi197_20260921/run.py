"""OI-197: OLD (pre-fix snapshot inputs) vs NEW (fixed production inputs), same engine and BASE. 14 starts, full/A/U, 0bp."""
import csv
import hashlib
import json
import os
...
def prepare():
    ...
    inputs |= {str(p.relative_to(ROOT)): digest(p) for p in [PANEL, EXP/'actions_corrected.csv', EXP/'price_terms_corrected.csv', EXP/'legacy_bridge_keys.json', EXP/'rebuild_rounding_audit.json']}
    for arm in ('OLD','NEW'):
