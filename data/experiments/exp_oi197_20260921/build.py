...
if len(sys.argv) > 1:
    ...
    ca.PRICE_TERMS_PATH = None if arm == 'OLD' else EXP/'price_terms_corrected.csv'
    ...
    if task == 'bands':
        ...
        module = types.ModuleType('oi197_build'); module.__file__ = str(path); sys.modules[module.__name__] = module
        # Freeze the unrelated minority bridge at the stored panel's convention.
        # The 156 explicit report keys predate 7bd97bef; both arms use its parent
        # implementation here, and assemble.py still requires exact OLD equality.
        source_text = source.read_text()
        needle = '                return minority_claims.equity_bridge(\n'
        assert source_text.count(needle) == 1
        legacy = '''                if (code, period) in OI197_LEGACY_BRIDGE_KEYS:
...
'''
        module.OI197_LEGACY_BRIDGE_KEYS = {tuple(k) for k in json.loads((EXP/'legacy_bridge_keys.json').read_text())}
        exec(compile(source_text.replace(needle, legacy + needle), str(source), 'exec'), module.__dict__)
