...
def scan(config,group,workers,exclude=None):
    command=[sys.executable,str(EXP/"scan.py"),str(EXP/config),
             ...
             "--workers",str(workers),"--title","operating横截面买入比例：每日前16%至24%"]
...
def main():
    ...
    checks = []
    for group in ("full", "A"):
        for arm, prior_arm in (("BASE", "BASE"), ("FIX110FR09", "BL11000OP09")):
    ...
    audit({"A": sorted(a), **{g: info["codes"] for g, info in union_groups.items() if not info["reused_A"]}})
    ...
    save("verification.json",{"completed_beijing":datetime.now(ZoneInfo("Asia/Shanghai")).isoformat(),
        ...
        "candidate_models":1,"candidate_combinations":len(labels)-2,"known_nonbase_controls":1,"metric":sw.METRIC_VERSION})
    assert unchanged
