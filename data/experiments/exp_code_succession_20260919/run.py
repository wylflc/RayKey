"""OI-192: OLD (pre-fix snapshot inputs) vs NEW (fixed production inputs), same engine and BASE. 14 starts, full/A/U, 0bp."""
import csv
import hashlib
import json
import os
...
OLD = EXP / 'old_inputs'
OLD_ARGS = ['--universe-file', str(OLD / 'panel_moat_bank_v6b.csv'), '--daily-states', str(OLD / 'a_share_daily_states_adopted.csv'),
            '--hold-states', str(OLD / 'a_share_daily_states_hold.csv')]
PROD_INPUTS = ['data/processed/pit_attention/panel_moat_bank_v6b.csv', 'data/processed/pit_attention/verdicts_pit_moat_v6.csv',
               'data/processed/a_share_daily_states_adopted.csv', 'data/processed/a_share_daily_states_hold.csv',
               'data/raw/corporate_actions/a_share_corporate_actions.csv', 'data/reference/a_share_code_succession.csv',
               'data/reference/equity_bond_csi300.csv', 'data/reference/cost_of_equity_inputs.csv',
               'data/raw/ohlcv/000022.csv', 'data/raw/ohlcv/001872.csv',
               'scripts/backtest_valuation_strategy.py', 'scripts/lot_cooldown.py', 'scripts/sweep_backtest_configs.py', 'scripts/code_succession.py']
...
def prepare():
    ...
    inputs |= {str(p.relative_to(ROOT)): digest(p) for p in sorted(EXP.glob('*.py')) + [EXP / 'preregister.md']}
    recorded = dict(line.split()[::-1] for line in (OLD / 'SHA256SUMS').read_text().splitlines())
    old = {}
    for name, h in recorded.items():
        p = ROOT / name
        old[name] = digest(p)
        assert old[name]['sha256'] == h, ('snapshot changed', name)
    states = json.loads((EXP / 'states_validation.json').read_text())
    # pre-fix production adopted states == snapshot (2bd535c9…, also the hash registered by exp_oi167_land / exp_oi180_189)
    assert old[str((OLD / 'a_share_daily_states_adopted.csv').relative_to(ROOT))]['sha256'] == '2bd535c9a4c6dea8e3ad52a2cd98b36511278ba12b2016d9b0eb0c5c04c88f68'
    assert states['after_sha256']['data/processed/a_share_daily_states_adopted.csv'] == inputs['data/processed/a_share_daily_states_adopted.csv']['sha256']
    ends = {}
    for code in ('000022', '001872'):
    ...
    manifest = dict(started_beijing=datetime.now(ZoneInfo('Asia/Shanghai')).isoformat(), job_id=os.getenv('SLURM_JOB_ID'),
                    base=sw.BASE, starts=sw.DEFAULT_STARTS, metric=sw.METRIC_VERSION, inputs=inputs, old_inputs=old,
                    old_args=OLD_ARGS, price_ends=ends)
...
def one(job):
    ...
    if arm == 'OLD':
        command += OLD_ARGS
        env['EXP_ACTIONS_FILE'] = str(OLD / 'a_share_corporate_actions.csv')
    else:
        env.pop('EXP_ACTIONS_FILE', None)
    if excluded:
