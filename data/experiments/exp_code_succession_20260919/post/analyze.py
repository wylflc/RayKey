"""Pair OLD/NEW per start and report the §12.1 Track A guardrail (OI-192, inputs-only change)."""
import contextlib
import csv
import json
...
def main():
    ...
    for group in groups[1:]:
        ...
        with (EXP / f'report_full_{group}.txt').open('w') as f, contextlib.redirect_stdout(f):
            sw.report(combined, 'OI-192 换码主体事件与面板修复：NEW（修复后输入）vs OLD（修复前快照），同引擎同 BASE，0bp（轨道 A 见 verification.json）')
