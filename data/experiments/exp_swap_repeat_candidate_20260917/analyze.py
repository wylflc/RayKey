...
def main():
    ...
    reference_path = EXP.parent / 'exp_stop_max_20260916/summary_rows.csv'
    with reference_path.open() as f:
        reference = {(r['group'], r['start']): r for r in csv.DictReader(f)
                     if r['arm'] == 'BASE'}
    for group, arms in groups.items():
        assert not sw.pairing_defects(arms, 'REPEAT', 'BASE', sw.DEFAULT_STARTS, group)
    ...
    verdict, reasons, values = sw.adoption_verdict(groups['full'], groups['A'], 'REPEAT')
    idx = {(r['group'], r['arm'], r['metric']): r for r in metrics}
    inferior = []
    for key, scale, good in CLAUSE4:
        delta = idx['U', 'REPEAT', key]['paired_delta_median'] * scale * good
...
def register():
    ...
    for r in rows:
        if r['group'] == 'U' and reused:
            continue
        # Cohorts belong in the batch, so the arms index counts one candidate.
        # Dedicated batch tags also protect all previously registered BASE rows.
        ledger_arm = 'BASE' if r['arm'] == 'BASE' else 'SWAPREPEAT'
        tag = (f"SWREPEAT{r['group']}20260917_{ledger_arm}{r['start'].replace('-', '')}"
    ...
    with ledger.ARMS_INDEX.open() as f:
        candidates = [r for r in csv.DictReader(f) if r['臂名'] == 'SWAPREPEAT']
    assert len(candidates) == 1 and int(candidates[0]['行数']) == (28 if reused else 42)
    save('registration.json', dict(summary_rows=56 if reused else 84,
         added_keys=len(after) - len(before), old_rows_unchanged=True,
         candidate_arm='SWAPREPEAT', candidate_arm_count=1,
