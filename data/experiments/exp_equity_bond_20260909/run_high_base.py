...
from prepare_high_base import fingerprint, save
sys.path.insert(0,str(ROOT/"scripts"))
import sweep_backtest_configs as sw


def scan(config,group,workers,exclude=None):
    command=[sys.executable,str(EXP.parent/"scan_high_base.py"),str(EXP/config),
...
def main():
    ...
    files=protected|{EXP.parent/"scan_high_base.py",EXP.parent/"run_high_base.py",EXP.parent/"high_base_engine.py",EXP/"configs.txt"}
    ...
    save("verification.json",{"completed_beijing":datetime.now(ZoneInfo("Asia/Shanghai")).isoformat(),
        ...
        "candidate_models":3,"candidate_combinations":7,"known_nonbase_controls":0,"metric":sw.METRIC_VERSION})
    assert unchanged
