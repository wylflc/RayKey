"""Run the preregistered operating buy-line scan with paired full/A/U controls."""
import argparse
import csv
...
def scan(config,group,workers,exclude=None):
    command=[sys.executable,str(EXP/"scan.py"),str(EXP/config),
             ...
             "--workers",str(workers),"--title","operating买入线独立扫描：不对齐合格比例"]
...
def main():
    ...
    # Compare against the current registered production baseline frozen before scanning.
    ...
    for group in ("full", "A"):
        for start, row in groups[group]["BASE"].items():
            tag = sw.summary_tag("BASE", start, "fixed" if group == "A" else "")
            prev = previous[tag]
            for field in (*sw.FIELDS, "前五赢家", "首个净值日", "末次净值日"):
                assert row[field] == prev[field], (group, start, field, row[field], prev[field])
            checks.append(f"{group}:{start}:47_report_fields_identical")
    ...
    save("verification.json",{"completed_beijing":datetime.now(ZoneInfo("Asia/Shanghai")).isoformat(),
        ...
        "candidate_models":1,"candidate_combinations":len(labels)-1,"metric":sw.METRIC_VERSION})
    assert unchanged
