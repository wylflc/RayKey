...
def main():
    ...
    ruling = json.loads((EXP / 'user_ruling.json').read_text()) if (EXP / 'user_ruling.json').exists() else {}
    assert v['zero_negative_cash'] and (v['track_a_pass'] or ruling.get('adopt_despite_guardrail'))
    arm = ruling.get('registered_arm', 'NEW')
    ...
    for r in rows:
        ...
        tmp = target.with_name('.' + target.name + '.oi200')
    ...
    (EXP / 'registration.json').write_text(json.dumps(dict(registered_base_rows=28, overwritten_keys=overwritten, new_keys=len(after) - len(before),
        input_epoch='nonop_cash_guarded_equity_20260923', precedent='OI-168 §12.217 同键重登', arm=arm, final_job=json.loads((EXP/'completed.json').read_text())['job_id'],
