...
from common import EXP, ROOT, digest, read
...
def main():
    ...
    if not any(r['run_id'] == run_id for r in read(DEFAULT_DECISION_LOG)):
        append_decision_log(DEFAULT_DECISION_LOG, [dict(
            ...
            summary_reason='OI-200～203：类金融按总资产 20% 识别走权益口径并补 λ 锚与峰谷守卫；非经营金融资产按账面计入、金融收益不进 EBIT、附注核定存款计现金；五粮液 2025 季报原列报版本与更正公告重述日；g0=0 记 none。用户 2026-09-23 裁定口径，轨道 A 记录见 verification.json。',
