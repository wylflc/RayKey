"""All preregistered full/A/U paths, isolated summaries and exact daily NAV."""
import argparse
...
def one(job):
    ...
    if not resume or not summary.exists():
        command = [sys.executable, str(EXP/'engine.py'), *shlex.split(sw.BASE), *states,
                   '--eb-policy', arm, '--since', start, '--label-suffix', '_'+tag,
...
def batch(group, arms, excluded, args, manifest):
    jobs = [(group, arm, start, excluded, manifest['state_args'], args.resume)
            for arm in arms for start in sw.DEFAULT_STARTS]
    rows = []
...
def main():
    ...
    if args.pilot:
        rows = [one(('pilot', a, '2011-11-01', [], manifest['state_args'], False))
                for a in ('BASE','C000R40','MA60X140')]
        save('pilot.json', rows)
    ...
    a = set(anchor['前五赢家'].split('/'))-{''}
    assert len(a) == 5
    rows = full + batch('A', labels, sorted(a), args, manifest)
    # Verify 28 current BASE paths against the landed production scan before interpreting candidates.
    prior = sw.load_scan(EXP.parent/'exp_ebds03_land_20260910/sweep_base.txt')[0]
    checks = 0
    for row in rows:
        if row['arm'] != 'BASE':
            continue
        old = prior['' if row['group']=='full' else 'EX5:']['BASE'][row['start']]
    ...
    for arm in labels[1:]:
        ...
        winner_sets[arm] = dict(A=sorted(a), B=sorted(b), U=list(u))
    ...
    save('verification.json', dict(completed_beijing=datetime.now(ZoneInfo('Asia/Shanghai')).isoformat(),
         ...
         baseline_values_checked=checks, baseline_paths=28, baseline_windows_identical=True,
         executed_paths=executed, summary_rows=len(rows), union_groups=union_groups))
    print(f'SCAN COMPLETE {executed} paths / {len(rows)} rows; BASE reproduced.', flush=True)
