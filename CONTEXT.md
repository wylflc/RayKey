...
- **Worth-Attention Company** — kept for ongoing attention. This set *is* the **Watchlist**; these receive a **Quality Tier** of L1–L3.
- **Documented-Not-Attention Company** — a company the user named for a dossier whose analysis found no moat carrier. It keeps a dossier, an L4 tier, a reference score and a valuation range, but is off the watchlist and is never buyable at any price tier. Re-entry to worth-attention goes through the same hard-trigger channel as boundary-pending (workflow §5.5).
...
**Quality Tier** — a business-quality rank assigned to worth-attention companies (L1–L3) and to documented-not-attention companies (always L4), ignoring current price. Current price belongs to **Valuation Assessment** and must never move a tier. Removal from research is expressed through **Attention Class**, not through a tier: an L3→L4 downgrade is an attention-class migration that carries its own evidence. Tier definitions and the tier's exact relationship to watchlist membership are governed by workflow §5.7/§5.8. The scored three-tier redesign went live on 2026-08-01 (workflow v1.27); its design trail is archived at `docs/archive/Ashare_tiering_v2_design.md`.
...
**Investment Strategy Tag** — the primary investment-case classification used for research grouping and display (cash-flow compounder, GARP growth, supply-chain breakout, fallen champion, resource NAV, cost-curve cyclical, financial capital, steady cash distribution, pipeline/R&D asset, subscription/deferred revenue, order-book realisation). It is not an industry, theme, or price-action label, and it does **not** select the production valuation model: every A-share production band goes through the single model in workflow §6.5.1; tag definitions live in the personal investment system §5.
...
- Every eligible company gets an **Attention Class**; worth-attention companies receive a **Quality Tier** L1–L3, documented-not-attention companies receive L4, boundary-pending and garbage companies carry no tier.
...
- An **Investment Strategy Tag** is assigned before valuation for research classification only; the valuation method is fixed by workflow §6.5.1 regardless of tag.
