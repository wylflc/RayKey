...
def main() -> int:
    ...
    if missing:
        # 无 ok 带 = 模型判不可估或数据不全 → 走 §6.5.7.4 手工带，不静默
        names = "、".join(pool[c] for c in missing)
        print(f"⚠ 无模型带 {len(missing)} 只（走 §6.5.7.4 手工带或建档队列）：{names}")
    return 0
