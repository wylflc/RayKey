...
* `tactical_thesis`（L3 9 家）——§6.2 对 L3 的买入前置是「须有明确战术理由兑现路径」。
...
`tactical_thesis` 允许并且**必须**能写「无」：§6.2 对 L3 的前置若不成立，如实写无，
...
# L3 战术理由（§6.2 买入前置）。每条要么给出**可证伪的兑现路径 + 里程碑**，要么写「无」。
L3_TACTICAL_THESIS = {
    "603156": "**无**。品类结构性退潮已在数上兑现（收入 −12%／利润 −27%），新品类拓展未验证，"
              "无可指的兑现里程碑；高分红属估值理由而非战术理由。按 §6.2 即使落低估档亦不可买。",
    "300146": "**无**。品牌壁垒正被线上白牌与跨境购侵蚀且**侵蚀正在发生**（连续两年收入下滑 −8.4%、"
              "ROE 降至 7.2%、药店渠道萎缩），无止跌路径。按 §6.2 即使落低估档亦不可买。",
    ...
    "300341": "**无**。供应链位置非垄断（ABB/西门子多供应商体系）、盈利平庸（ROE 9.1%）、业务组合分散；"
              "对照华明装备无同等品类垄断位与回报证据。按 §6.2 即使落低估档亦不可买。",
...
def main() -> int:
    ...
    print(f"  其中 L3 判「无战术理由」{no_thesis} 家 —— 按 §6.2，这几家即使落低估档也不可买")
