#!/usr/bin/env python3
"""海外关注清单逐票估值档案与参考分（§6.8 + §6.5.2 + §5.7.4）。
...
1. **倍数直接指定或用历史 PE 中位**（11/19 有带者）——违反 §6.5.2 v1.54「倍数必须
...
2. **带宽由方法自身区间叠加**——PEG [1.0,1.5] 被直接当成带宽，泡泡玛特/谷歌/优步/
   ...
   §6.5.2 v1.62 明写「带宽一律由分层系数承担，不由方法自身的区间叠加」。
3. **PEG 未写 ROE 修正理由**（6 家）——§6.5.2 v1.54 要求写明取值理由。
...
------------------------------
海外清单 2026-08-06 新增伯克希尔，是本清单第一家**保险/金融控股**公司。§6.5 判定顺序
...
# §6.2 定档阈值只有一处实现，海外清单不另写一份（CLAUDE.md：标准不得重述）。
...
# 分层系数（§6.5.2 v1.62：带宽只由这里来）。L1-L3 与 A 股同表；L4 是海外清单独有
...
class Company:
    ...
    def band(self) -> tuple[float | None, float | None, str, str]:
        ...
        if self.path == "peg":
            ...
            # 而其 ROE 与 g 隐含的可持续派息率是 50%、PE 14x）。终值期分红率 §6.5.2 本就
            ...
            # 倍数被推导而非指定（§6.5.2 v1.54）；带宽仍只由分层系数承担（v1.62）。
...
def materialize(as_of: str) -> None:
    ...
    for row in rows:
        ...
        # 审定档由本次的带与复核时点价重算（§6.2），不沿用旧值：带一改而档不改，
