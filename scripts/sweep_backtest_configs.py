...
# （在册合格面 17.960%/30.465%，508,453 个 (代码,日) 观测）→ v4.33 起 **0.94/2.50/0.15**（见下）。
...
# **v4.33 三线取两位小数**（2026-08-21 用户裁定，结 OI-079，回测日志 §12.102）：对齐解 0.9434/2.5008/0.1451 → 生产 **0.94/2.50/0.15**
# （`--width 0.06 --sell-line 2.50 --swap-margin 0.15`）；合格面 17.823%/30.460% 与四位小数解只差 0.1pp 观测，0.01 步长邻域读数差 ±1pp 内、
# 符号混合、减持线 2.49~2.52 逐位无差——四位小数解是单点峰不取，以后每次重解同样只保留两位小数。**新基准在册读数 滚3/滚5/逐年中位
# 41.33/42.58/37.44、逐年正 89%、滚3回撤 35.7%、换手 4.56、仓位 98%**；此后配对差一律相对该读数（v4.32 的 40.58/43.42/35.61 是四位小数线上的过渡读数）。
...
BASE = (
    ...
    "--width 0.06 --sell-line 2.50 --swap-margin 0.15 "
