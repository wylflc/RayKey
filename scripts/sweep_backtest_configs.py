...
# **三条线的沿革**：v4.20 分红折算纪元 0.9528/2.5690/0.1466 → v4.23 留存时序修正纪元对齐解 0.9407/2.5236/0.1447
# （在册合格面 17.960%/30.465%，508,453 个 (代码,日) 观测）→ v4.34 起 **0.9434/2.5008/0.1451**（见下）。
# **v4.28 公告日口径纪元**（2026-08-21）：逐日状态改为带自可得日前一市场交易日起生效（与生产当晚吸收同构），状态文件全量重建、
# 带与三线不变（重解减持线 2.5222 与 2.5236 逐笔无差），新基准在册读数 滚3/滚5/逐年中位 38.13/40.89/42.07、换手 5.60、
# 滚3回撤 39.1%（回测日志 §12.97.1）。
# **v4.32 缺陷修复纪元**（2026-08-21，回测日志 §12.101）：①回测引擎均线改前复权折回当日口径、除权日折算止损锚与峰价
# （`--ma-basis adjusted --exright-stop adjust`，缺省，OI-054——此前现金分红日假止损整仓清空）；②逐日状态按公告日封顶口径
# 全量重建（`build_historical_valuation_bands.py --notice-cap statutory`，缺省，OI-042 建带侧）。**新基准在册读数 滚3/滚5/逐年中位
# 40.58/43.42/35.61、逐年正 88%、滚3回撤 35.3%、换手 4.45、仓位 98%**；此后配对差一律相对该读数，v4.31 前的 Δ 读数属旧纪元、不跨纪元迁移。
# **v4.34 三线 = 封顶纪元对齐解 0.9434/2.5008/0.1451、保留四位小数**（2026-08-21 用户裁定，结 OI-079，回测日志 §12.101.3/§12.102）：
# 合格面 17.968%/30.443%（511,488 个在册观测）。两位小数取整 0.94/2.50/0.15 实测比对齐解低 0.72pp 滚5（4/23），用户以「损害年化」否决，
# 重解后不取整（v4.33 的两位小数落地当日撤回）。**新基准在册读数 滚3/滚5/逐年中位 41.18/43.86/37.58、逐年正 88%、滚3回撤 35.6%、
# 换手 4.55、仓位 98%**；此后配对差一律相对该读数（v4.32 的 40.58/43.42/35.61 是旧三线上的过渡读数）。
# **v4.41 资金顺序纪元**（2026-08-22，OI-081 用户裁定并同日纠正，回测日志 §12.111）：授信每日按净资产重定、**所有**卖出款（含换仓）先还超额负债、
# 负债回到额度内才可买入、换仓与买入按现金＋剩余授信判（`--swap-trigger power --credit-over-limit repay`，缺省）。**新基准在册读数 滚3/滚5/逐年中位
# 39.71/42.10/30.76、年化中位 33.67、逐年正 88%、滚3回撤 35.0%、最大回撤中位 52.0%、换手 4.29、仓位 98%、Sharpe 0.79**；此后配对差一律相对该读数
# （v4.34 的 41.18/43.86/37.58 是旧资金顺序上的读数；不融资对照 35.95/38.84/34.19、年化中位 31.55、最大回撤中位 40.1）。
# **v4.44 采纳纪元**（2026-08-22 用户裁定：授信 60% 不封顶 ＋ 涨幅减持 125%，回测日志 §12.113 G60 臂／§12.114）：
# **新基准在册读数 滚3/滚5/逐年中位 54.18/63.18/37.85、年化中位 47.59（P10 33.03）、逐年正 83%、滚3回撤 51.4%、
# 最大回撤中位 61.9%、换手 7.76、仓位 98%、Sharpe 0.84、Calmar 0.76；2011-11 长跑 期末 5.36 亿、年化 42.28%／最大回撤 65.2%**；
# 此后配对差一律相对该读数（v4.41 的 39.71/42.10/30.76 是 180 万封顶、无涨幅减持的读数）。
# **v4.56 保险股利折现纪元**（2026-08-23，OI-085 用户裁定①，回测日志 §12.117）：保险与银行同走股利折现，回测宇宙内只有中国平安（2008-2022）换口径，
# BASE 命令不变；**新基准在册读数 滚3/滚5/逐年中位 51.72/61.67/35.93、年化中位 47.06（P10 30.68）、逐年正 86%、滚3回撤 51.7%、
# 最大回撤中位 62.8%、换手 7.71、仓位 98%、Sharpe 0.83、Calmar 0.75；2011-11 长跑 年化 38.78%／最大回撤 66.4%**；此后配对差一律相对该读数。
# **v4.59 股本口径纪元**（2026-08-23，OI-086/OI-087 用户裁定「正确性优先于收益率」，回测日志 §12.118）：每股锚改为
# 「归一化比率 × 经营账面」、外生权益按面值进净现金、送转窗口自 `bps_basis_date` 起算，状态文件全量重建、BASE 命令与三线不变；
# **新基准在册读数 滚3/滚5/逐年中位 37.22/53.92/46.61、年化中位 41.61（P10 29.09）、逐年正 83%、滚3回撤 49.7%、最大回撤中位 70.8%、
# Sharpe 0.72、Calmar 0.56、换手 10.10、仓位 97%；2011-11 长跑 年化 38.24%／最大回撤 82.0%**；此后配对差一律相对该读数
# （v4.56 的 51.72/61.67/35.93 含 OI-087 缺陷：格力 2015 中报带按除权前股本列示、P/V 0.26 实为 0.54，旧基准借此重仓格力拿到的路径）。
# 对齐三线解 0.9503/2.5238/0.1462 已于 v4.60 落地（用户裁定），BASE 读数 = §12.118 NEWL 臂。
# **v4.61 连续判据纪元**（OI-088，回测日志 §12.119）：周期守卫坡道 w＋增长态分级 λ（`--roic-cond-detect graded --roic-peak-ramp 0.3`），状态全量重建、三线重解 0.9505/2.5263/0.1462；在册读数见 §12.119。
# **v4.62 季度当期化纪元**（OI-089/090/091，回测日志 §12.120）：季报 TTM 当期化＋增速腿折减＋谷底对称守卫＋薄权益守卫（`--ttm-current on --growth-damp on --thin-equity-max 0.5`），
# 状态全量重建、三线重解 0.9343/2.4671/0.1437；**新基准在册读数 滚3/滚5/逐年中位 63.57/63.47/63.14、年化中位 54.08（P10 43.15）、逐年正 86%、滚3回撤 47.8%、
# 最大回撤中位 64.0%、Sharpe 0.95、Calmar 0.89、换手 8.24、仓位 99%、逐年最差 −26.2%；2011-11 长跑 年化 46.57%／最大回撤 64.2%**；此后配对差一律相对该读数。
# **v4.64 单票上限 60% 纪元（2026-08-23 用户裁定，回测日志 §12.123）**：新基准在册读数 **滚5 中位／P25／最差 61.93／55.74／29.47、
# 滚5 回撤中位 47.1、滚5 Calmar 1.36、滚5 Sharpe 1.20；滚3 中位 53.90／Calmar 1.27／Sharpe 1.16／回撤 44.0；年化中位 48.50、最大回撤中位 61.5、
# Calmar 0.90、Sharpe 0.94、换手 8.03、逐年中位 45.43、逐年最差 −22.0；2011-11 长跑 年化 43.4%／最大回撤 61.5%**；此后配对差一律相对该读数。
# **v4.63 读数口径（2026-08-23，用户裁定；回测日志 §12.121）**：滚动窗口月末锚定，决策读数四项（滚5 中位 Δ／滚5 P25 Δ／滚5 回撤闸门／负窗否决）。
# 同一 v4.62 策略的在册读数改记为 **滚5 中位／P25／最差 63.93／54.33／34.01、滚5 回撤中位 48.2、滚5 Calmar 1.31、滚5 Sharpe 1.10；
# 滚3 中位 64.16、滚3 Sharpe 1.16；年化中位 54.08、最大回撤中位 64.0、Sharpe 0.95、Calmar 0.89、换手 8.24**（旧口径滚5 63.47／滚3 63.57，同曲线噪声级差）。
...
BASE = (
    ...
    # **回测基础设置＝融资口径**（用户 2026-08-17 裁定，2026-08-22 改为不封顶）：本金 300 万、授信 = 净资产×60%
    # （v4.44 起**不设金额上限**；v4.05~v4.43 为 min(净资产×60%, 180 万)）、强平线 130%、融资年利率 3.5%。
