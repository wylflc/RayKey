...
BASE = (
    ...
    # 持仓侧（减持线／换仓来源／簇内升级／T+1 换仓确认）改读 max(BASE,B2) 状态（`--hold-states`），减持线按持仓侧
    # 同一上侧面重解为 2.4257，换仓边际 0.1437 不动（§12.136.2：减持线 2.4257~2.4671 逐位无影响、边际重解噪声级）。
    "--width 0.0657 --sell-line 2.4257 --swap-margin 0.1437 "
