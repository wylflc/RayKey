...
def _print_group(arms, order: list[str], failed, title: str) -> None:
    ...
    def longrun(label: str) -> str:
        """两个长跑锚点的全期 CAGR；该锚点不在本轮起点集里就留空。"""
        out = ""
        for s in LONGRUN_STARTS:
            row = arms[label].get(s)
            out += f"{row['年化'] * 100:>10.2f}" if row else f"{'—':>10}"
        return out

    print("【辅助读数】只描述不排序：滚 3 看短中期失效，滚 10 只在够长的起点上有值（空≠差）；两个长跑锚点是单起点水平值、常反号，比较两臂一律走决策表的 Δ年化（逐起点配对）；"
          "未来年化的水平引用只用全期口径（年化／5年块中位／长跑锚点），滚动重叠中位的水平只描述窗口分布")
    print(f"{'配置':<14}{'Δ滚3':>8}{'符号':>7}{'Δ逐年':>8}{'符号':>7}"
          f"{'滚3年化':>8}{'滚3Calmar':>10}{'滚3Sharpe':>10}{'滚3回撤':>8}{'滚10年化':>9}{'起点数':>7}"
          f"{'年化':>7}{'5年块中位':>10}{'长跑09-11':>10}{'长跑11-11':>10}"
          f"{'最大回撤':>9}{'Calmar':>8}{'Sharpe':>8}{'逐年中位':>9}{'逐年正':>7}{'逐年最差':>9}")
    for _sort, label, dz, n, med, _neg in rows:
        ...
        print(f"{label:<14}{cells}"
              ...
              f"{_fmt(med('年化'), 100, 7)}{_fmt(med('互不重叠5年块中位'), 100, 10)}{longrun(label)}{_fmt(med('最大回撤'), 100, 9, 1)}{_fmt(med('Calmar'), 1, 8)}{_fmt(med('Sharpe'), 1, 8)}"
