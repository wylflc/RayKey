...
# **v4.40 资金顺序纪元**（2026-08-22，OI-080 用户裁定，回测日志 §12.109）：授信每日按净资产重定、常规卖出款先还超额负债、换仓与买入按
# 现金＋剩余授信判（`--swap-trigger power --credit-over-limit repay`，缺省）。**新基准在册读数 滚3/滚5/逐年中位 38.05/42.54/34.95、逐年正 87%、
# 滚3回撤 35.2%、最大回撤中位 52.1%、换手 4.30、仓位 98%、Sharpe 0.82**；此后配对差一律相对该读数（v4.34 的 41.18/43.86/37.58 是旧资金顺序上的读数）。
...
BASE = (
    ...
    # **§10.2 资金顺序（v4.40，OI-080 用户 2026-08-22 裁定）**：授信额度每日按净资产重定；当日卖出后负债超出额度的部分
    # 先用现金偿还、不可新增买入（`--credit-over-limit repay`）；换仓与买入都按「现金＋剩余授信」判资金是否够一档
    # （`--swap-trigger power`）。两者都是缺省，此处显式写出以防缺省漂移；v4.39 前旧口径 = `cash` + `keep`。
