...
BASE = (
    ...
    "--swap-gain-once "
    # **v4.135 换仓接收方守卫 K=1（用户 2026-09-03 裁定采纳 SB1）**：当日换仓卖出源不进买入队列，候选侧 P/V 高于
    # 「最低换仓源持仓侧 P/V − 0.15」的候选一并剔除；回测日志 §12.179 `SB1` 臂（对 v4.133 +0.52／+0.07，去赢家 +1.01／+0.59）、
    # 平台检验与用户裁定见 §12.181。`--swap-source-block -1` 为旧口径。
    "--swap-source-block 1 "
