...
（不在起点集时取最早起点）的 summary 读 `前五赢家`（按代码汇总逐日「盈亏 ÷ 前一日净资产」累计贡献
的前五名，引擎日末记账；名义盈亏会被复利放大的末期账户体量主导），把同一组代码用 `--exclude-codes` 从**全部臂**统一剔除再跑一遍，结果行以 `EX5:` 前缀
...
BASE = (
    ...
    # **v4.132 相关性上限取消**（用户 2026-09-02 裁定「0.7 不能有效降回撤则取消」，回测日志 §12.174 表 S）：
    # `--max-corr 1.0` = 相关性照算、无一被跳过；生产 SEC93_MAX_CORR 同值、只列报告。v4.131 前为 0.70。
    ...
    "--lot-size 100 --lot-ratio-cooldown --lot-cooldown-start confirmed --execution-consistency signal --exec-delay 1 --exec-price close "
    ...
    # 同日采纳**涨幅减持** `--gain-sell 1.25 --gain-sell-mode gated`（§12.110/§12.113）。
    # **v4.132 涨幅减持 1.10 ungated**（用户 2026-09-02 裁定「接受再平衡形态」，§12.174 表 S／T／U／V）：收盘 ≥ 持仓均价×2.10
    # 即减一档、不看走势，并优先作换仓卖出源（同样不要求弱势）。生产落点 SEC93_GAIN_SELL（扫描器常量）＋ track_holdings_daily 提示。
    ...
    "--gain-sell 1.10 --gain-sell-mode ungated "
    # **v4.134 同日一档（OI-142，用户 2026-09-03 裁定）**：当日已涨幅减持的持仓不作换仓卖出源，同一持仓每日合计至多减一档；
    # 回测日志 §12.179 `GSO` 臂（−0.11／+0.45，去赢家 +1.06／0.00，噪声级零代价）。`--no-swap-gain-once` 为旧口径。
    "--swap-gain-once "
    # 换仓接收方守卫 `--swap-source-block K`：-1 = 关（现行）。K=1 曾作 v4.135 生产口径（回测日志 §12.179／§12.181），
    # 相对贡献尺下去赢家表 −1.17／−0.56、全样本零（§12.182），用户 2026-09-03 裁定回退（§12.183）。研究臂改 gate／rank-mode 时仍须显式给 -1。
    "--swap-source-block -1 "
    ...
    # **v4.120 换仓边际 0.18**：不随买入线缩放，按 0.01 一档剂量扫描在 W=0 标度上重定；平台 0.13~0.18 见回测日志 §12.163，OI-128 口径下 12 档无平台、维持（§12.167）。
    # **v4.132 换仓边际 0.16**：相关性取消＋涨幅减持 ungated 的组合臂上 0.10~0.30 重扫，全样本平台 0.13~0.18、0.19 起悬崖、
    # 去赢家单调偏向更低边际，取平台严格端（用户裁定，§12.174 表 R）。
    # **v4.133 换仓边际 0.15**：0.16 的配对差为正但水平中位低于 0.18，用户复核 §12.175 末段后改取平台内水平中位最高的 0.15。
