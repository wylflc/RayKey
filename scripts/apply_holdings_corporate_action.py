...
用法::
    ...
    # 实付现金与交易所价格减项分别给值（--cash 从 v4.197 起只表示实付）
    python3 scripts/apply_holdings_corporate_action.py --as-of 2026-09-24 --code 603529 --cash .392 --price-cash .385 --ratio 0
...
配股认购（缺省认购，`--no-subscribe` 不认购）增加 `原股数 × r`。同一 (代码, 除权日) 已登记即拒绝再次执行。
"""
...
from corporate_actions import AMOUNTS, PRICE_FIELDS, event_from_row, with_price_terms
...
def event_from_actions(path: Path, code: str, as_of: str) -> dict[str, float] | None:
    ...
    return dict(cash=event['cash_per_share'], ratio=event['share_ratio'],
                ...
                price=tuple(event[k] for k in PRICE_FIELDS))
...
def event_from_eastmoney(code: str, as_of: str, timeout: float) -> dict[str, float] | None:
    ...
    return {"cash": float(ev["cash_per_share"]), "ratio": float(ev["share_ratio"]),  # type: ignore[arg-type]
            ...
            "price": tuple(ev[k] for k in PRICE_FIELDS)}
...
def apply_action(holdings_path: Path, ledger_path: Path, log_path: Path | None, code: str, as_of: str,
                 ...
                 subscribe: bool, source: str, note: str = "", dry_run: bool = False,
                 price: tuple | None = None) -> dict[str, object]:
    code = code.zfill(6)
    ...
    actual = dict(zip(AMOUNTS, (cash, ratio, rights_ratio, rights_price)))
    if price is None:
        verified = with_price_terms(dict(security_code=code, ex_dividend_date=as_of, **actual))
        price = tuple(verified[k] for k in PRICE_FIELDS)
    event = event_from_row({**actual, **dict(zip(PRICE_FIELDS, price))})
    price = event.price
    denom_shares = 1 + ratio + (rights_ratio if subscribe else 0.0)
    shares1 = shares0 * denom_shares
    cost1 = adjust_for_ex_dividend(cost0, *price) if cost0 is not None else None
    stop1 = adjust_for_ex_dividend(stop0, *price) if stop0 is not None else None
    result = {"security_code": code, "security_name": row.get("security_name", ""), "ex_dividend_date": as_of,
              "cash_per_share": f"{cash:.12g}", "share_ratio": f"{ratio:.12g}", "rights_ratio": f"{rights_ratio:g}",
              ...
              **{k: format(v, '.12g') for k, v in zip(PRICE_FIELDS, price)}, "amount_basis": "actual_and_exchange"}
    if dry_run:
        return result
    ...
    if old_header and 'amount_basis' not in old_header:
        # Preserve the old recorded numbers; their cash column was price-only.
        with ledger_path.open('w', newline='', encoding='utf-8') as fh:
            w = csv.DictWriter(fh, fieldnames=LEDGER_FIELDS); w.writeheader()
            w.writerows({**r, 'amount_basis': 'legacy_price_only'} for r in ledger)
...
def main() -> int:
    ...
    ap.add_argument("--cash", type=float, help="每股实付税前现金（v4.197 起；价格减项另用 --price-cash）")
    ...
    ap.add_argument("--price-cash", type=float, help="交易所除息现金减项")
    ap.add_argument("--price-ratio", type=float, help="交易所虚拟送转比例")
    ap.add_argument("--price-rights-ratio", type=float, help="交易所配股比例")
    ap.add_argument("--price-rights-price", type=float, help="交易所配股价")
    ...
    if explicit:
        ...
        verified = with_price_terms(dict(security_code=code, ex_dividend_date=args.as_of,
            **dict(zip(AMOUNTS, (ev['cash'], ev['ratio'], ev['rights_ratio'], ev['rights_price'])))))
        ev['price'] = tuple(verified[k] for k in PRICE_FIELDS)
        source = "explicit"
    else:
    ...
    ev['price'] = tuple(v if override is None else override for v, override in zip(ev['price'],
        (args.price_cash, args.price_ratio, args.price_rights_ratio, args.price_rights_price)))
    result = apply_action(args.holdings, args.ledger, None if args.dry_run else args.log_file, code, args.as_of,
                          ...
                          not args.no_subscribe, source, args.note, args.dry_run, price=ev['price'])
