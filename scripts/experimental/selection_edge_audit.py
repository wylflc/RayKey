#!/usr/bin/env python3
"""选择环节的信息量审计（L2 信号层：`docs/000_Ashare_workflow.md` §12.1 第 10 款）。

回测的路径读数（滚 5 中位等）由少数复利段主导，臂间差异常常只反映「谁更妥善地
碰到了那几个赢家」。本脚本改问候选集内的描述性问题（重复观测仍可能集中于少数赢家）：
...
* 表 2 **排序信息量**——合格集名次（rank 1 / 2-5 / 6-10）对前向总回报的单调性。
  按名次汇总全部股票×日期观测，包含重复入选和已持仓候选，不含资金分配。
* 表 3 **换仓的方向性**——同日「换仓卖出源」对「换仓买入目标」的前向总回报配对。
  换仓的全部理由是把钱押回更便宜的档，这张表检验该动作事后是否成立。

**统计口径**：表 1／3 先在日内取中位、再跨日汇总；表 2 直接按名次合并
全部股票×日期观测，不做周期去重。同一股票持续入选会得到更大权重。
各表的前向窗口均可能重叠，原始行数、不同配对数、年度同号均不证明样本独立或因果。
按首次信号至 MA60 破位计一次的对照见 `pv_episode_forward.py`。
...
def main() -> int:
    ...
    print_block(f"表 1　边际选择检验（合格集内 held=0：买到的 − 没买到的，前向 {args.horizon} 日总回报）",
                res1, "正值表示该样本中买入组的前向表现较高；不单独证明预测能力或因果增益。")

    # ---- 表 2：排序信息量
    print(f"\n表 2　排序信息量（合格集名次 → 前向 {args.horizon} 日总回报，股票×日期观测、未按周期去重）")
    ...
    print_block(f"表 3　换仓方向性（同日：换入目标 − 换出源，前向 {args.horizon} 日总回报）",
                res3, f"正值表示该样本中触发目标的前向表现较高；不等于实际资金流的净超额收益。\n"
    ...
    print("\n口径提示：表1/3日内先取中位再跨日汇总；表2按名次合并全部股票×日期观测，未按周期去重。"
          "\n持续入选会增加同一股票权重；前向窗口重叠，日数/配对数/年度同号不代表独立性或因果。"
          "\n首次触发至MA60破位的去重对照见 pv_episode_forward.py。")
    return 0
