...
**生产取值只保留两位小数**（v4.33，用户 2026-08-21 裁定，回测日志 §12.102）：对齐解按本脚本重解后四舍五入到 0.01
再落到 §9.3.1／`SEC93_*`／`BASE`——实测 0.01 步长邻域的读数差异为路径噪声量级（±1pp 内、符号混合、减持线 2.49~2.52
逐位无差），四位小数解是单点峰；合格面按取整后的线重新读出即可（0.94/2.50 为 17.823%/30.460%，与 0.9434/2.5008 只差 0.1pp 观测）。
"""
import argparse
import bisect
import collections
...
def main():
    ...
    for path in (a.arms or [a.base]):
        ...
        if a.swap_margin is not None:
            ...
            swap_aligned = a.swap_margin * buy / a.base_line
            print(f"  → 生产取两位小数（v4.33 惯例）：买入线 {round(buy, 2):.2f}（`--width {round(1 - round(buy, 2), 2):.2f}`）／减持线 {round(sell, 2):.2f}／换仓改善 {round(swap_aligned, 2):.2f}")
