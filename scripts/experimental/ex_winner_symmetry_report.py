...
def main() -> None:
    ...
    for line in args.sweep.read_text(encoding="utf-8").splitlines():
        if line.startswith("#SET|"):
            ...
            sets[tag] = codes
    groups, _orders, failed, _note, version, _fields = load_scan(args.sweep)
    if any(failed.values()):
        raise ValueError("剂量/对称性结果含失败路径，不能报告为完整检验")
    for full, starts in groups["EX5:"].items():
        tag = max((t for t in sets if full.startswith(t)), key=len, default=None)
        if tag is not None:
            rows[(tag, full[len(tag):])] = starts
    if not sets:
        raise ValueError("缺少剔除集声明")
    print(f"候选 {args.challenger}；计量 {version}；剔除集：" + "；".join(f"{t} = {c.replace(',', '/')}" for t, c in sets.items()))
    ...
    for tag in sets:
        ...
        if not base or set(base) != set(arm):
            raise ValueError(f"{tag}: BASE 与 {args.challenger} 起点不齐")
        starts = sorted(base)
        cells = []
        ...
        for key, scale, good in CLAUSE4:
            ...
            noise = 0.005 if scale == 1 else 0.15
            if d < -noise:
                bad.append(f"{key} {d:+.3f}" if scale == 1 else f"{key} {d:+.2f}pp")
