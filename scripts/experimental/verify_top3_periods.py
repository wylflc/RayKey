...
def main():
    ...
    latched=manifest.get('eligibility')=='stop-latched'
    ...
    for r in rows:
        ...
        assert float(r['ma20'])>float(r['ma60'])
        if not latched:assert float(r['signal_close'])>float(r['ma20'])
        ix=days.index(day)+1
    ...
    if latched:
        from verify_top3_latch import verify_latched
        rank_dates,ma_checks,activation_checks=verify_latched(manifest,byday,days,signal_days,quotes,raw,actions,direct_ma,OUT)
        sample_days=range(rank_dates)
    else:
    ...
    if latched:groups['activation_first_top3']=read('activation_first_top3.csv')
    for h in (1,3):
    ...
    if latched:
        from verify_top3_latch import verify_ma60_counting
        verify_ma60_counting(rows,groups['ma60_episodes'],quotes,raw,actions,manifest['cutoff'])
        reference=OUT/'first_run_primary_hashes.json'
        if reference.exists():
            for name,value in json.loads(reference.read_text()).items():
                assert sha(OUT/name)==value,('primary_changed_after_auxiliary_fix',name)
    ...
    if latched:
        result['activation_cycles_checked']=activation_checks
        result['ma60_counting_checked']=len(groups['ma60_episodes'])
