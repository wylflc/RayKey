...
def display_cells(row: dict[str, str], quote: dict | None) -> dict[str, object]:
    ...
    # 2026-08-23 用户指令：阅读版改列**模型价值 V**（= 区间中值 = 模型内在价值）与 **P/V**（§3 定义：ROIC 路径为
    # (现价+每股净负债)÷每股企业价值、其余 现价÷V，与 §9.3 同一口径、三位小数），模型价值区间／空间两列移出阅读版（仍在 CSV）。
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=["skip", "fetch"],
        default="fetch",
        # **缺省 fetch（2026-08-17 改）**：阅读版的现价/模型价值/`P/V` 三列都依赖行情快照，
