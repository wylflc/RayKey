...
| 版本 | 日期 | 内容 |
| --- | --- | --- |
| v4.199 | 2026-09-24 | OI-200～203、OI-208：§6.5.1 路径 3 按金融中介负债 ≥ 总资产 20% 识别类金融、走权益口径并补 λ 锚／TTM／峰谷守卫（`--equity-anchor guarded`），客户资金不计现金；非经营金融资产按账面计入、年报附注核定存款计现金（`fetch_cash_note_items.py` → `data/reference/cash_note_items.csv`），金融收益不进 EBIT，财务公司吸存／回购／短融计债（`--cash-caliber nonop`）；g0 ≤ 0 记 `none`，⑤腿只在权重 > 0 时计入；§6.3 重述：面板原列报版本 `data/reference/panel_restatement_originals.csv`，更正公告扫描 `scan_restatement_announcements.py` 推迟未分版本期，§6.7 第 1 步与盘后作业接入，§7.4 增重述事件；§3 等处 V 改称模型价值，`intrinsic_value.valuation_label` 四档改为相对模型价值的位置标签、美股状态共用；§9.3.1 买入线 1.0454 → 1.0562（`SEC93_BUY_LINE`、`BASE --width`），换仓边际 0.15 不变；依据 §12.258 |
| v4.198 | 2026-09-21 | OI-198：§6.5.1 明示普通少数股东扣减零下界，保留账面符号；`minority_claims.equity_bridge` 显式执行同一规则，历史状态与下游按生产桥同步，旧 BASE 输入独立冻结；依据 §12.255 |
...
| v4.190 | 2026-09-18 | 新增§10.4独立影子组合：冻结规则、原日输入与代码指纹，保留初始持仓并按T+1递推；隔离模拟成交、净值与实盘差额归因，区分历史回放和前向观察。落点`shadow_portfolio.py`及回测可选持仓种子接口，交易规则不变。依据§12.247 |
| v4.189 | 2026-09-16 | §9.2 增加不筛走势的候选侧 P/V 前十观察表；扫描器正式输出、行情预览与阅读日志同步展示排名及走势状态，交易规则不变。落点 `screen_daily_volume_price_signals.py` |
| v4.188 | 2026-09-14 | §12.1 第 7 款滑点压力测试改为仅用户明确要求时运行，第 8 款按适用要求检查；保留代码与功能，默认 0bp。依据 §12.240 |
