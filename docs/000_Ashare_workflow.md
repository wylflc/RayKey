...
**回购与分红的处理（两侧同口径）**：估值只看「可分配现金 = NOPAT × (1 − 维持增长所需留存)」，分红与回购同属可分配现金、不区分、不另按股数缩减重复计量，未来回购计划不进模型；回购的实际影响是**通过缩小账面权益抬高 NOPAT/权益与 ROIC0**——回购注销超过净利润的公司（Adobe、英伟达、苹果）每股 NOPAT 锚会被低估、峰值守卫可能误判为周期峰（OI-082，待裁定）；现金堆积公司反向高估（拼多多）。A 股回购规模小、分红按 §11.4 除权归一化处理，银行股利折现只计现金股利。
