...
| 空间 | `V ÷ 现价 − 1`，仅作阅读展示 |
| 合格集 | 通过 §9.3.1 买入线、走势条件、冻结、流动性、L3 战术闸门与相关性过滤后的股票，按 `P/V` 升序 |
...
**主体不可比**——重组、资产注入、分拆或借壳使旧财务主体不可比。

**模型无法计算不构成手工带理由**：亏损、归一化 ROE 非正、`ROE_T` 贴近 `g_T` 的护栏拒绝、零增长股权价值 ≤ 0、最新 ok 模型带早于 `2025-01-01` 时点门槛——一律判「无法估值」：档案带清空、池内可见、带显示 —、无 `P/V`、不进 §9.3 任何判定；模型重新可算后自动回归模型带，无需人工动作。

手工带必须使用前瞻一致预期或归一化利润，倍数必须可推导并记录来源，能够双向支持买入与减持；不得因“看起来更合理”绕过模型。主体统一、模型重新可算后立即切回生产带。

**手工带必须同时落到生产带文件**（扫描器的 `P/V` 只读 `data/processed/a_share_pool_model_bands_adopted.csv`，逐票档案不是 §9.3 的输入）：落点是 `data/processed/manual_band_overrides.csv`，由 `apply_forecast_band_overlay.py` 在 §6.7 第 4 步之后写入生产带，并被 `apply_model_bands_to_dossiers.py` 同步到档案。覆盖表逐行必填 `reason_code`、`note`（含倍数推导与来源）与 `expires_when`（失效条件）。
...
| 买入线 | `P/V ≤ 0.9343` |
| 新建仓走势 | T 日 `收盘 > MA20 > MA60`；**成交日（T+1）收盘 < 当日 MA60 时该笔放弃**、资金顺位下一名 |
...
| 换仓 | 资金不足时：先换出**满足涨幅减持条件**的持仓一档（涨幅最大者，不比 `P/V` 边际）；否则候选 `P/V` 至少低 `0.1437`，且被换出持仓 `收盘 < MA20`；卖一档、买一档 |
| 止损 | 锚 = 零股建仓时的成交日 MA60；**生效止损线 = min(锚, 当日 MA60)**——均线下移时跟随下移、上移不抬线；收盘跌破生效线即整仓清空 |
| 止盈 | 无 |
...
回测基准的融资口径：本金 300 万；授信 = 净资产 × 60%，不设金额上限；强平线 130%；融资年利率 3.5%；资金顺序按 §10.2（授信每日按净资产重定，所有卖出款先偿还超出额度的负债，负债回到额度内才可买入，换仓与买入按现金＋剩余授信判）。实盘的授信额度按 §2 台账实际值（§10.2），回测授信随净资产放大是成文差异。

#### 9.3.2 每日机械顺序

1. 计算全池当日 `P/V`，排除不在名单、流动性不足、`review_pending` 与 L3 无战术理由（§9.3.1 L3 战术闸门）股票。
...
4. 先卖：
   ...
   - 想买但资金不足且满足换仓条件：先减持涨幅 ≥125% 且弱势的持仓一档（涨幅最大者），否则减持最贵的弱势持仓一档。
   - 任何减档后的余仓不足一档时清空。
5. 后买：对过滤后的合格集逐个买一档，按手向下取整，可用资金耗尽即停。成交日收盘低于当日 MA60 的**新建仓**放弃该笔、顺位下一名（§9.3.1 走势行；加仓不受此限）。
...
**生效止损线每日取 `min(锚, 当日同周期均线)`**：锚永不上移，均线跌到锚下时生效线跟随均线；价格低于生效线时首日即清，不引入等待日。持仓表只存锚不存周期（§11.2 五列），`track_holdings_daily.py` 统一按当日 MA60（前复权口径，§8.3）计算生效线。成交日已破 MA60 的建仓直接跳过（§9.3.1 走势行），新锚恒为 MA60；存量持仓若残留 MA20 锚，按同周期人工复核。
...
账户级只有两条外生硬约束——券商授信额度与 130% 强平线，口径见个人投资体系 §4。授信额度与融资负债读 §2 的账户快照台账（`credit_line_cny` 列）。每日实际可用资金为：
...
**带的调整由建带链机械维护**：生产带文件在 §6.7 第 4 步末段按「带的公告日之后的除权事件」归一化到现价口径（现金＋送转；银行与保险走股利折现、分子逐日滚动，不参与减项），回测逐日状态同一规则；调整持续到公告日晚于除息日的下一份报告接管为止，重建不抹掉。**持仓表仍是人工侧**：`entry_stop_price` 是历史价格，须按上表调整并永久保留；`cost_basis` 与股数同批调整。差异化分派按交易所价格口径处理。
...
换估值口径或换宇宙做 A/B 时，**三条线必须一起重解到同一在册合格面**（`scripts/experimental/align_buy_line.py`），否则比的是两条不同宽度的闸门。现行三线 **0.9343／2.4671／0.1437** 为 v4.62 纪元对齐解（下侧 17.617%／上侧 30.856%，511,416 个在册观测），保留四位小数、不取整。现行基准（`BASE` 含 `--position-cap 0.6`，月末锚定口径）在册读数：**滚5 中位／P25／最差 61.93／55.74／29.47、滚5 回撤中位 47.1、滚5 Calmar 1.36、滚5 Sharpe 1.20、负窗口占比 0；滚3 中位 53.90；年化中位 48.50、最大回撤中位 61.5、Calmar 0.90、Sharpe 0.94、换手 8.03、逐年中位 45.43、逐年最差 −22.0**。配对差一律相对现行基准读数，读数不跨纪元迁移；各纪元的三线解与在册读数只查 `docs/Ashare_backtest_log.md`。
