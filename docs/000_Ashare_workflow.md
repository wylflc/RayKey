...
| 换仓 | 只由**未持仓**的合格候选触发（已持仓候选加仓不触发），按 `P/V` 升序逐个判：可用资金不足一档时先换出**满足涨幅减持条件**的持仓一档（涨幅最大者，不比 `P/V` 边际）；否则该候选（候选侧 `P/V`）须比最贵的弱势持仓（持仓侧 `P/V`）至少低 `0.18`，且被换出持仓 `收盘 < MA20`；卖一档、买一档。卖出款不定向给触发候选，与其它可用资金一并按 §9.3.2 第 5 步 `P/V` 升序买入 |
...
一手金额大于一档时仍成交一手。令 `x = 一手金额 ÷ 一档`，随后跳过 `round(x) − 1` 次该票的合格机会；买入、涨幅减持和换仓共用同一计数器。冷却按合格次数，不按自然日。计数器持久化在 `data/processed/daily_cooldown_state.csv`，扫描器每次生成执行清单时读入、消费、回写；同一信号日重跑从 `remaining_before` 重算，`--as-of` 早于文件 `applied_trade_date` 的历史重放不应用也不回写。
...
4. 候选臂在剔除集 U 下、标准指标集每一项的配对差均不劣（配对差中位方向为正，或正号起点数不低于半数；长跑锚点、年均换手与集中度表不计入），记为**去赢家全面优秀**，登记为待考察候选并写入 `docs/Ashare_workflow_open_issues.md`。该判定不构成采纳，也不放宽第 2 款的门槛。采纳前须另行补齐三项：① 换仓边际在该臂上按 0.01 一档重新剂量扫描，不得沿用 `BASE` 的边际；② 剔除赢家只数取 1／3／5／10 的剂量曲线，四档读数须同向；③ 第 10~12 款读数齐备。三项齐备后按第 8 款处理。
