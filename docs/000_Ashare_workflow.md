...
公司行动明细按独立分配方案保存，保留代码、报告期、预案公告日与除权日；同日不同报告期或不同方案不得互相覆盖。相同方案的完全重复返回只计一次，同批同键金额冲突须报错；完整且非空的重取成功后以新明细替换该代码旧分红明细，预案转实施不保留重复预案，失败或空返回不清旧记录。源端跨报告期重复须以原公告核验，排除规则登记 `data/reference/a_share_action_component_exclusions.csv`（逐组件键、预期金额、来源与理由）；源金额变化时停止并复核，不静默沿用排除。原实施公告证明源端实付金额错误时，组件修正登记 `data/reference/a_share_action_component_corrections.csv`（完整组件键、原金额、正确金额、来源及理由）；原金额或已修正金额均可幂等刷新，第三种金额须停止复核，修正不丢报告期归属。除权价格、持仓现金流与影子快照按代码及除权日汇总：现金、送转比与配股比分别相加，配股认购款为各笔配股比乘配股价之和；同日只作一次除权，跨日才复合。报告期分红归属及预案可得日计算仍读取逐笔明细，不使用按日合计。明细键、去重与按日汇总唯一实现为 `scripts/corporate_actions.py`，取数、换码复制与各消费者共用。
...
回测的执行与公司行动口径：T+1 成交日无价（停牌、末日）该笔跳过、不回落信号日成交（与 §9.1 同）；除权按 §11.4 同式折算，配股全额认购、认购款不足部分计融资负债；现金红利按除权日计入现金（不模拟到账日，成文差异）；差别化股息税按卖出时持有期对所卖股份持有期内已收现金红利结算（≤1 个月 20%、≤1 年 10%、>1 年免，FIFO；送股面值部分不计，成文差异）；差异化分派的实付与价格参数按 §11.4 分流。
...
公司行动分为两组参数：明细中的 `cash_per_share`、`share_ratio`、`rights_ratio`、`rights_price` 始终表示普通股东实付现金、实际获配股份及认购款；股息入账、税基及含权收益读取这组参数。不同股东获派金额不同时，公司权益桥不得把公众股东每股实收乘以全公司股数；组件修正表另存 `accounting_cash_per_share`（公告分派总额÷除权前公司总股本）供 `dividends_total`／`dividends_booked_since` 使用，普通股东股息收益仍用实收字段。交易所除权参考价的参数另存于 `data/reference/a_share_exright_terms.csv`，按代码＋除权日覆盖**当天合计**，字段为 `price_cash_per_share`、`price_share_ratio`、`price_rights_ratio`、`price_rights_price`；同时保存预期实付合计、实施公告日期、来源及页码。仅原实施公告可确立覆盖值，保留公告精度；预期实付合计不匹配、重复键或公告日晚于除权日须报错。未覆盖事件沿用实付参数，并在覆盖审计中列为未核验，不能据此声称没有差异。历史价格、均线、估值带、止损与价格口径成本读取价格参数；实际股数及现金不得使用虚拟分派值。读取及校验唯一实现为 `scripts/corporate_actions.py`；仅在除权日生效，公告已知但尚未除权不得提前调整。

除权日按交易所除权参考价同一公式调整；下表价格列使用价格参数、股数列使用实际参数：
...
python3 scripts/apply_holdings_corporate_action.py --as-of YYYY-MM-DD --code <代码> --cash <每股实付> --ratio <实际送转> --price-cash <除息减项> --price-ratio <虚拟送转>   # --cash 为实付；交易所价格参数分别给值
```
