...
换估值口径或换宇宙做 A/B 时，**买入线必须重解到同一在册合格面**（`scripts/experimental/align_buy_line.py`），否则比的是两条不同宽度的闸门；**换仓边际不随买入线缩放，按 0.01 一档的剂量扫描重定**。现行两线 **候选侧买入线 0.9976／换仓边际 0.18**：买入线对候选侧状态下侧合格面 17.777%，508,154 个在册观测，保留四位小数、不取整。现行基准（`BASE`：候选侧 `a_share_daily_states_adopted.csv`＋持仓侧 `a_share_daily_states_hold.csv`、授信 66.6%、单票上限 60%、T+1 无价跳过、股息税、换仓源同日不重复、同日买卖对冲、配股事件，月末锚定口径）在册读数（全样本）：**滚5 中位／P25／最差 65.02／52.67／18.79、滚5 回撤中位 46.6、滚5 Calmar 1.39、滚5 Sharpe 1.20、负窗口占比 0；年化中位 45.70、最大回撤中位 64.4、Calmar 0.71、Sharpe 0.86；5年块中位 57.19；长跑 2009-11 CAGR 39.22／MDD 64.6、2011-11 CAGR 45.53／MDD 64.1；滚3 中位 55.45、滚3 回撤中位 46.4；逐年中位 39.95、逐年最差 −18.0；换手 5.77、仓位 167**。在册读数（去赢家，剔除集 A ＝ 002128／002466／601328／601225／688516）：**滚5 中位／P25／最差 39.16／32.49／21.94、滚5 回撤中位 46.4、滚5 Calmar 0.85、滚5 Sharpe 0.81、负窗口占比 0；年化中位 38.19、最大回撤中位 63.4、Calmar 0.59、Sharpe 0.78；5年块中位 46.92；长跑 2009-11 CAGR 31.26／MDD 63.3、2011-11 CAGR 38.22／MDD 63.8；滚3 中位 41.46、滚3 回撤中位 41.8；逐年中位 35.14、逐年最差 −23.3；换手 5.94、仓位 166**。剔除集 U 随候选臂而变，不入册。配对差一律相对现行基准读数，读数不跨纪元迁移；各纪元的线解与在册读数只查 `docs/Ashare_backtest_log.md`。
