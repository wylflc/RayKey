...
换估值口径或换宇宙做 A/B 时，**三条线必须一起重解到同一在册合格面**（`scripts/experimental/align_buy_line.py`），否则比的是两条不同宽度的闸门。现行三线 **0.9343／2.4671／0.1437** 为 v4.62 纪元对齐解（下侧 17.617%／上侧 30.856%，511,416 个在册观测），保留四位小数、不取整。现行基准（`BASE`：授信 66.6%、单票上限 60%、T+1 无价跳过、股息税、换仓源同日不重复、配股事件，月末锚定口径）在册读数：**滚5 中位／P25／最差 59.77／48.61／26.39、滚5 回撤中位 47.7、滚5 Calmar 1.28、滚5 Sharpe 1.12、负窗口占比 0；滚3 中位 52.26；年化中位 45.68、最大回撤中位 59.4、Calmar 0.71、Sharpe 0.84、换手 4.29、逐年中位 46.38、逐年最差 −26.7**。配对差一律相对现行基准读数，读数不跨纪元迁移；各纪元的三线解与在册读数只查 `docs/Ashare_backtest_log.md`。
