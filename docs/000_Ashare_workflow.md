...
换估值口径或换宇宙做 A/B 时，**三条线必须一起重解到同一在册合格面**（`scripts/experimental/align_buy_line.py`），否则比的是两条不同宽度的闸门。现行基准的合格面是下侧 17.960%、上侧 30.465%（v4.23 留存时序修正纪元在 508,453 个在册观测上重解，三线 0.9407/2.5236/0.1447；v4.20 分红折算纪元为 0.9528/2.5690/0.1466）；各研究臂已解出的线记在 `docs/Ashare_backtest_log.md` §12.83.1。**v4.32 纪元**（公告日封顶状态＋前复权均线／折算锚引擎，回测日志 §12.101）：在册观测 511,488，对齐解 0.9434/2.5008/0.1451 登记 OI-079 待用户裁定，生产三线暂不动；新基准在册读数以 §12.101.4 的 NEW 臂为准。
