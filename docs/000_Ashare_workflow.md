...
2. 预先按预计实盘持有期选收益主窗口，缺省取 5 年。滚动窗口**月末锚定**：窗口末日 = 每个自然月最后一个交易日，首日 = 60 个月前同月末，年化按实际日历年数，回撤与 Sharpe 用窗口内逐日净值。标准起点集 = 路径长度 ≥ 10 年的全部半年档起点（现 14 个：2009-11-01 ~ 2016-05-01；数据末端推进使新档满 10 年时补入该档并重登在册读数）。标准起点集上**决策读数只有五项**：主读数 = 滚动 5 年 CAGR 中位的配对差中位与正号数；复利读数 = **全期 CAGR** 的配对差中位与正号数；坏情形 = 滚动 5 年 CAGR P25（**起点内**月末窗口的分位；P10 不作决策）的配对差与正号数；闸门 = 滚动 5 年回撤中位不得变深超过 3pp；否决 = 滚动 5 年负收益窗口占比由 0 转正（过半起点）。五项一律取全样本表的读数。**主读数与复利读数任一为负即不采纳**。**臂间比较与「未来年化表现」的表述基准一律为复利读数**，对照表按 Δ年化 排序。未来年化的水平引用只用全期口径读数——年化（全期 CAGR）、互不重叠 5 年块中位、长跑年化；滚动重叠窗口的水平值只描述窗口分布，不作未来预期引用。Δ 读数与符号数描述已过历史，不作未来 Δ 的点预测引用。其余一律只描述不排序：互不重叠 5 年块中位（自最新窗口末月往回每 60 个月一窗、首尾相接零重叠，取中位）、滚动 5 年最差值、滚动 3 年 CAGR／Calmar／Sharpe、滚动 10 年 CAGR、逐年收益分布、最大回撤／Calmar／Sharpe。不得看完结果再换主窗口或换读数。同时报告换手、平均仓位（＝持仓市值 ÷ 净资产，融资下可超 100%）、集中度表（持仓数、单票权重中位／P90／最大、前三权重中位、单票超 60% 天数占比）与**长跑年化**——2009-11-01 与 2011-11-01 两个锚点起点的全期 CAGR，两个都报，只描述不排序；两臂比较一律取逐起点配对差，不比较两个水平值。两层分位不要混：符号数是 14 个起点层，P25／最差是起点内窗口层。
3. 报逐起点配对差中位和正号起点数，不比较两个独立中位数的差。每轮扫描同时出第二张对照表：统一剔除 `BASE` 前五大赢家（`BASE` 臂 2011-11-01 起点全部闭合周期按代码汇总 `proceeds − invested` 的前五名，取 summary `前五赢家` 列；`--exclude-codes` 从全部臂统一剔除，不改面板），Δ 对去赢家 `BASE` 配对；`sweep_backtest_configs.py` 缺省自动跑第二遍，`--no-ex-top5` 只用于纯补跑。
...
换估值口径或换宇宙做 A/B 时，**买入线必须重解到同一在册合格面**（`scripts/experimental/align_buy_line.py`），否则比的是两条不同宽度的闸门；**换仓边际不随买入线缩放，按 0.01 一档的剂量扫描重定**。现行两线 **候选侧买入线 0.9343／换仓边际 0.19**：买入线对候选侧状态下侧合格面 17.777%，508,154 个在册观测，保留四位小数、不取整。现行基准（`BASE`：候选侧 `a_share_daily_states_adopted.csv`＋持仓侧 `a_share_daily_states_hold.csv`、授信 66.6%、单票上限 60%、T+1 无价跳过、股息税、换仓源同日不重复、同日买卖对冲、配股事件，月末锚定口径）在册读数：**滚5 中位／P25／最差 65.23／47.37／18.25、5年块中位 57.03、滚5 回撤中位 48.3、滚5 Calmar 1.35、滚5 Sharpe 1.17、负窗口占比 0；滚3 中位 51.18；年化中位 46.09、长跑年化 36.92（2009-11）／40.47（2011-11）、最大回撤中位 64.0、Calmar 0.72、Sharpe 0.87、换手 4.75、逐年中位 43.43、逐年最差 −28.4**。配对差一律相对现行基准读数，读数不跨纪元迁移；各纪元的线解与在册读数只查 `docs/Ashare_backtest_log.md`。
