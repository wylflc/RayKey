...
| 换仓 | 只由**未持仓**的合格候选触发（已持仓候选加仓不触发），按 `P/V` 升序逐个判：可用资金不足一档时先换出**满足涨幅减持条件**的持仓一档（涨幅最大者，不比 `P/V` 边际、不要求弱势）；否则该候选（候选侧 `P/V`）须比最贵的弱势持仓（持仓侧 `P/V`）至少低 `0.15`，且被换出持仓 `收盘 < MA20`；卖一档、买一档。卖出款不定向给触发候选，与其它可用资金一并按 §9.3.2 第 5 步 `P/V` 升序买入 |
...
换估值口径或换宇宙做 A/B 时，**买入线必须重解到同一在册合格面**（`scripts/experimental/align_buy_line.py`），否则比的是两条不同宽度的闸门；**换仓边际不随买入线缩放，按 0.01 一档的剂量扫描重定**。现行两线 **候选侧买入线 1.0454／换仓边际 0.15**：买入线对候选侧状态下侧合格面 17.771%，508,260 个在册观测，保留四位小数、不取整。现行基准（`BASE`：候选侧 `a_share_daily_states_adopted.csv`＋持仓侧 `a_share_daily_states_hold.csv`、授信 66.6%、单票上限 60%、相关性只报告不过滤、涨幅减持 110% 不看走势、T+1 无价跳过、股息税、换仓源同日不重复、同日买卖对冲、配股事件，月末锚定口径）在册读数（全样本）：**滚5 中位／P25／最差 50.37／37.62／11.40、滚5 回撤中位 47.1、滚5 Calmar 1.07、滚5 Sharpe 0.96、负窗口占比 0.0；年化中位 45.78、最大回撤中位 70.0、Calmar 0.62、Sharpe 0.88；5年块中位 62.78；长跑 2009-11 CAGR 34.59／MDD 68.9、2011-11 CAGR 32.60／MDD 76.1；滚3 中位 47.74、滚3 回撤中位 43.9；逐年中位 41.27、逐年最差 −25.1；换手 6.93、仓位 165**。在册读数（去赢家，剔除集 A ＝ 688516／000933／002128／601328／600919）：**滚5 中位／P25／最差 53.35／40.10／12.20、滚5 回撤中位 44.1、滚5 Calmar 1.22、滚5 Sharpe 1.10、负窗口占比 0；年化中位 40.81、最大回撤中位 64.2、Calmar 0.61、Sharpe 0.82；5年块中位 59.87；长跑 2009-11 CAGR 32.83／MDD 69.4、2011-11 CAGR 38.25／MDD 63.6；滚3 中位 48.81、滚3 回撤中位 41.6；逐年中位 47.59、逐年最差 −23.2；换手 5.87、仓位 165**。剔除集 U 随候选臂而变，不入册。配对差一律相对现行基准读数，读数不跨纪元迁移；各纪元的线解与在册读数只查 `docs/Ashare_backtest_log.md`。
