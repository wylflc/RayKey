...
影子组合与实盘分别记账，落点 `data/processed/shadow_portfolio/e4/`。读取该目录 `protocol.json` 的基准日、冻结配置和代码指纹；日输入只追加到 `snapshots/`，不得用今日估值、名单或实盘持仓覆盖旧输入与影子状态。
...
3. 当日生产扫描与实际成交回写完成后，执行 `python3 scripts/shadow_portfolio.py capture --as-of YYYY-MM-DD`，再执行 `python3 scripts/shadow_portfolio.py run`。输出逐日净值、模拟成交、当前持仓、实盘差额归因和阅读报告；模拟成交不得写入实盘持仓或实盘冷却凭据。
4. 建立日前的区间标为历史回放，建立后的完整交易日才标为前向观察。实质规则变化后分新纪元，不改旧配置；代码指纹变化须先复核并显式建立新记录。未取得初始批次历史的股息税、实盘未分类费用与券商实时额度差异分别列示，不计作策略超额收益。
