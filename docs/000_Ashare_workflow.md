...
| 买入线 | `P/V ≤ 0.9343` |
| 新建仓走势 | T 日 `收盘 > MA20 > MA60`；**T 日收盘 < 成交日（T+1）MA60 时该笔放弃**、资金顺位下一名 |
| 已有持仓加仓走势 | `MA20 > MA60`，不要求收盘高于 MA20 |
| 排序 | `P/V` 升序，资金用尽即停 |
| 相关性 | 与在手及已选标的近 252 日相关性 `≤ 0.70`；超限跳过，最多下扫 40 名 |
| 流动性 | §10.1 第 3 条的成交额门槛，在合格集硬执行（阈值只在 §10.1 定义）。回测不复现（成文差异） |
...
1. 计算全池当日 `P/V`，排除不在名单、流动性不足、`review_pending`（回测不复现，成文差异）与 L3 无战术理由（§9.3.1 L3 战术闸门）股票。
...
4. 先卖：
   ...
   - 任何减档后的余仓不足一手时清空。
5. 后买：对过滤后的合格集逐个买一档，按手向下取整，可用资金耗尽即停。成交日 MA60 高于 T 日收盘的**新建仓**放弃该笔、顺位下一名（§9.3.1 走势行；加仓不受此限）。
...
**生效止损线每日取 `min(锚, 当日同周期均线)`**：锚永不上移，均线跌到锚下时生效线跟随均线；执行时点现价低于生效线时当日即清，不引入等待日。持仓表只存锚不存周期（§11.2 五列），`track_holdings_daily.py` 统一按当日 MA60（前复权口径，§8.3）计算生效线，收盘跌破时提示次日尾盘按当日线复核。T 日收盘低于成交日 MA60 的建仓直接跳过（§9.3.1 走势行），新锚恒为成交日 MA60；存量持仓若残留 MA20 锚，按同周期人工复核。
...
换估值口径或换宇宙做 A/B 时，**三条线必须一起重解到同一在册合格面**（`scripts/experimental/align_buy_line.py`），否则比的是两条不同宽度的闸门。现行三线 **0.9343／2.4671／0.1437** 为 v4.62 纪元对齐解（下侧 17.617%／上侧 30.856%，511,416 个在册观测），保留四位小数、不取整。现行基准（`BASE`：授信 66.6%、单票上限 60%，月末锚定口径）在册读数：**滚5 中位／P25／最差 64.41／54.91／29.69、滚5 回撤中位 48.1、滚5 Calmar 1.37、滚5 Sharpe 1.18、负窗口占比 0；滚3 中位 56.46；年化中位 50.91、最大回撤中位 63.4、Calmar 0.91、Sharpe 0.95、换手 9.09、逐年中位 45.71、逐年最差 −24.1**。配对差一律相对现行基准读数，读数不跨纪元迁移；各纪元的三线解与在册读数只查 `docs/Ashare_backtest_log.md`。
