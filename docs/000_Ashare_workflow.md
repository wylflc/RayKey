...
**带的调整由建带链机械维护**：生产带文件在 §6.7 第 4 步末段按「带的公告日之后的除权事件」归一化到现价口径（现金＋送转；银行与保险走股利折现、分子按财年归属，不参与减项），回测逐日状态同一规则；调整持续到公告日晚于除息日的下一份报告接管为止，重建不抹掉。**持仓表仍是人工侧**：`entry_stop_price` 是历史价格，须按上表调整并永久保留；`cost_basis` 与股数同批调整。差异化分派按交易所价格口径处理。
...
换估值口径或换宇宙做 A/B 时，**三条线必须一起重解到同一在册合格面**（`scripts/experimental/align_buy_line.py`），否则比的是两条不同宽度的闸门。现行三线 **0.9343／2.4671／0.1437** 为 v4.62 纪元对齐解（下侧 17.617%／上侧 30.856%，511,416 个在册观测），保留四位小数、不取整。现行基准（`BASE`：授信 66.6%、单票上限 60%，月末锚定口径）在册读数：**滚5 中位／P25／最差 61.55／54.34／29.09、滚5 回撤中位 48.1、滚5 Calmar 1.30、滚5 Sharpe 1.15、负窗口占比 0；滚3 中位 56.27；年化中位 48.81、最大回撤中位 61.8、Calmar 0.86、Sharpe 0.91、换手 4.93、逐年中位 42.92、逐年最差 −24.4**。配对差一律相对现行基准读数，读数不跨纪元迁移；各纪元的三线解与在册读数只查 `docs/Ashare_backtest_log.md`。
