...
| 换仓 | 只由**未持仓**的合格候选触发（已持仓候选加仓不触发），按 `P/V` 升序逐个判：可用资金不足一档时先换出**满足涨幅减持条件**的持仓一档（涨幅最大者，不比 `P/V` 边际、不要求弱势）；否则该候选（候选侧 `P/V`）须比最贵的弱势持仓（持仓侧 `P/V`）至少低 `0.15`，且被换出持仓 `收盘 < MA20`；卖一档、买一档；当日已涨幅减持的持仓不作卖出源，同一持仓每日合计至多减一档。卖出款不定向给触发候选，与其它可用资金一并按 §9.3.2 第 5 步 `P/V` 升序买入；当日买入队列剔除换仓卖出源，以及候选侧 `P/V` 高于「当日最低换仓源持仓侧 `P/V` − 0.15」的候选（涨幅让位源只剔除自身） |
...
4. 先卖：
   ...
   - 任何减档后的余仓不足一手时清空。
5. 后买：当日有换仓卖出时，先从合格集剔除换仓卖出源与候选侧 `P/V` 高于「最低换仓源持仓侧 `P/V` − 0.15」的候选；对余下合格集按 `P/V` 升序逐个买一档（含换仓卖出款，不定向给触发候选），按手向下取整，可用资金耗尽即停。
...
11. 采纳候选报 `scripts/experimental/delta_attribution.py` 的前三只净额占比；超过 100% 者不作采纳依据。
...
换估值口径或换宇宙做 A/B 时，**买入线必须重解到同一在册合格面**（`scripts/experimental/align_buy_line.py`），否则比的是两条不同宽度的闸门；**换仓边际不随买入线缩放，按 0.01 一档的剂量扫描重定**。现行两线 **候选侧买入线 1.0454／换仓边际 0.15**：买入线对候选侧状态下侧合格面 17.771%，508,260 个在册观测，保留四位小数、不取整。现行基准（`BASE`：候选侧 `a_share_daily_states_adopted.csv`＋持仓侧 `a_share_daily_states_hold.csv`、授信 66.6%、单票上限 60%、相关性只报告不过滤、涨幅减持 110% 不看走势、T+1 无价跳过、股息税、换仓源同日不重复、同日一档、换仓接收方守卫、同日买卖对冲、配股事件，月末锚定口径）在册读数（全样本）：**滚5 中位／P25／最差 48.75／40.10／11.50、滚5 回撤中位 46.5、滚5 Calmar 1.07、滚5 Sharpe 0.96、负窗口占比 0.0；年化中位 46.58、最大回撤中位 69.4、Calmar 0.63、Sharpe 0.88；5年块中位 61.53；长跑 2009-11 CAGR 30.35／MDD 69.3、2011-11 CAGR 33.06／MDD 75.1；滚3 中位 48.22、滚3 回撤中位 43.9；逐年中位 38.22、逐年最差 −27.2；换手 7.07、仓位 165**。在册读数（去赢家，剔除集 A ＝ 688516／000933／002128／601328／600919）：**滚5 中位／P25／最差 56.43／38.01／12.80、滚5 回撤中位 43.5、滚5 Calmar 1.24、滚5 Sharpe 1.20、负窗口占比 0；年化中位 42.03、最大回撤中位 62.9、Calmar 0.63、Sharpe 0.85；5年块中位 61.89；长跑 2009-11 CAGR 40.24／MDD 65.1、2011-11 CAGR 38.42／MDD 62.3；滚3 中位 53.30、滚3 回撤中位 41.9；逐年中位 53.13、逐年最差 −25.2；换手 5.96、仓位 165**。剔除集 U 随候选臂而变，不入册。配对差一律相对现行基准读数，读数不跨纪元迁移；各纪元的线解与在册读数只查 `docs/Ashare_backtest_log.md`。
