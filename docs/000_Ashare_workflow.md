...
生产 `P/V` 与回测 `valuation_ratio` 必须逐位一致（除 §6.4 叠加行这一成文例外）。早于 `2025-01-01` 的陈旧模型带不进交易层：扫描器与档案层同一阈值排除，该票无 `P/V`、不进 §9.3 判定（OI-068）。
...
| 持仓只数上限 | 无 |
| 单票机械上限 | 无 |
| 减持 | `P/V ≥ 2.5690` 且 `收盘 < MA20`，减一档 |
| 换仓 | 资金不足时，候选 `P/V` 至少低 `0.1466`，且被换出持仓 `收盘 < MA20`；卖一档、买一档 |
...
按 §9.3.1 的止损口径，由零股建仓时把实际成交日的止损基准写入 `entry_stop_price`。加仓不重设，价格上涨不上移，清仓后重新建仓才重设；除权除息按 §11.4 同因子调整。
...
换估值口径或换宇宙做 A/B 时，**三条线必须一起重解到同一在册合格面**（`scripts/experimental/align_buy_line.py`），否则比的是两条不同宽度的闸门。现行基准的合格面是下侧 17.960%、上侧 30.465%（v4.20 分红折算纪元在 508,453 个在册观测上重解，三线 0.9528/2.5690/0.1466）；各研究臂已解出的线记在 `docs/Ashare_backtest_log.md` §12.83.1。
