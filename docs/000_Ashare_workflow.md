...
4. 先卖：
   ...
   - 想买但资金不足且满足换仓条件：先减持涨幅 ≥125% 且弱势的持仓一档（涨幅最大者），否则减持最贵的弱势持仓一档；同一持仓每日至多换出一档，无其他卖出源即停止换仓。
   - 任何减档后的余仓不足一手时清空。
...
换估值口径或换宇宙做 A/B 时，**三条线必须一起重解到同一在册合格面**（`scripts/experimental/align_buy_line.py`），否则比的是两条不同宽度的闸门。现行三线 **0.9343／2.4671／0.1437** 为 v4.62 纪元对齐解（下侧 17.617%／上侧 30.856%，511,416 个在册观测），保留四位小数、不取整。现行基准（`BASE`：授信 66.6%、单票上限 60%、T+1 无价跳过、股息税、换仓源同日不重复，月末锚定口径；配股事件补齐前按无配股臂暂登）在册读数：**滚5 中位／P25／最差 64.03／51.31／28.84、滚5 回撤中位 48.3、滚5 Calmar 1.31、滚5 Sharpe 1.18、负窗口占比 0；滚3 中位 54.91；年化中位 49.14、最大回撤中位 59.4、Calmar 0.85、Sharpe 0.91、换手 5.31、逐年中位 42.73、逐年最差 −26.7**。配对差一律相对现行基准读数，读数不跨纪元迁移；各纪元的三线解与在册读数只查 `docs/Ashare_backtest_log.md`。
