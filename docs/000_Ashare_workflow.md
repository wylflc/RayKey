...
换估值口径或换宇宙做 A/B 时，**买入线必须重解到同一在册合格面**（`scripts/experimental/align_buy_line.py`），否则比的是两条不同宽度的闸门；**换仓边际不随买入线缩放，按 0.01 一档的剂量扫描重定**。现行两线 **候选侧买入线 1.0454／换仓边际 0.15**：买入线对候选侧状态下侧合格面 17.771%，508,260 个在册观测，保留四位小数、不取整。现行基准（`BASE`：候选侧 `a_share_daily_states_adopted.csv`＋持仓侧 `a_share_daily_states_hold.csv`、授信 66.6%、单票上限 60%、相关性只报告不过滤、涨幅减持 110% 不看走势、T+1 无价跳过、股息税、换仓源同日不重复、同日一档、换仓接收方守卫、同日买卖对冲、配股事件，月末锚定口径）在册读数（全样本）：**滚5 中位／P25／最差 51.20／38.39／11.51、滚5 回撤中位 46.4、滚5 Calmar 1.06、滚5 Sharpe 0.97、负窗口占比 0.0；年化中位 45.78、最大回撤中位 69.2、Calmar 0.60、Sharpe 0.87；5年块中位 61.48；长跑 2009-11 CAGR 31.50／MDD 69.4、2011-11 CAGR 36.09／MDD 74.5；滚3 中位 48.50、滚3 回撤中位 43.8；逐年中位 38.73、逐年最差 −27.2；换手 7.01、仓位 165**。在册读数（去赢家，剔除集 A ＝ 688516／000933／002128／002648／601328）：**滚5 中位／P25／最差 53.96／38.28／12.23、滚5 回撤中位 44.1、滚5 Calmar 1.27、滚5 Sharpe 1.13、负窗口占比 0；年化中位 38.88、最大回撤中位 64.9、Calmar 0.60、Sharpe 0.82；5年块中位 59.16；长跑 2009-11 CAGR 33.17／MDD 64.9、2011-11 CAGR 36.63／MDD 62.3；滚3 中位 50.49、滚3 回撤中位 41.7；逐年中位 48.92、逐年最差 −25.2；换手 5.66、仓位 165**。剔除集 U 随候选臂而变，不入册。配对差一律相对现行基准读数，读数不跨纪元迁移；各纪元的线解与在册读数只查 `docs/Ashare_backtest_log.md`。
