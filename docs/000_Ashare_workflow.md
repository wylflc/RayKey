...
3. 报逐起点配对差中位和正号起点数，不比较两个独立中位数的差。每轮扫描同时出第二张对照表：统一剔除 `BASE` 前五大赢家（`BASE` 臂 2011-11-01 起点全部闭合周期按代码汇总 `proceeds − invested` 的前五名，取 summary `前五赢家` 列；`--exclude-codes` 从全部臂统一剔除，不改面板），Δ 对去赢家 `BASE` 配对；`sweep_backtest_configs.py` 缺省自动跑第二遍，`--no-ex-top5` 只用于纯补跑。轨道 B 采纳要求全样本与去赢家两表的主读数同号；反号即判本轮无决策信息、生产不改。去赢家读数不单独作采纳依据。
