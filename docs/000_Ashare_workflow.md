...
| 持仓只数上限 | 无 |
| 单票机械上限 | 无 |
| 减持 | `P/V ≥ 2.50` 且 `收盘 < MA20`，减一档 |
| 换仓 | 资金不足时，候选 `P/V` 至少低 `0.15`，且被换出持仓 `收盘 < MA20`；卖一档、买一档 |
...
换估值口径或换宇宙做 A/B 时，**三条线必须一起重解到同一在册合格面**（`scripts/experimental/align_buy_line.py`），否则比的是两条不同宽度的闸门。现行基准的合格面是下侧 17.960%、上侧 30.465%（v4.23 留存时序修正纪元在 508,453 个在册观测上重解，三线 0.9407/2.5236/0.1447；v4.20 分红折算纪元为 0.9528/2.5690/0.1466）；各研究臂已解出的线记在 `docs/Ashare_backtest_log.md` §12.83.1。**v4.32/v4.33 纪元**（公告日封顶状态＋前复权均线／折算锚引擎，回测日志 §12.101/§12.102）：在册观测 511,488，对齐解 0.9434/2.5008/0.1451；**生产三线取对齐解四舍五入到两位小数 0.94/2.50/0.15（用户 2026-08-21 裁定，结 OI-079）**——合格面 17.823%/30.460%与四位小数解（17.968%/30.443%）只差 0.1pp 的观测，而 0.01 步长邻域的读数差异为 ±1pp 内、符号混合、减持线 2.49~2.52 逐位无差，四位小数解是单点峰（本条第 4 款）不取；以后每次重解同样只保留两位小数。新基准在册读数以 §12.102 的 L2 臂为准。
