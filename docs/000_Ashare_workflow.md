...
| 换仓 | 只由**未持仓**的合格候选触发（已持仓候选加仓不触发），按 `P/V` 升序逐个判：可用资金不足一档时先换出**满足涨幅减持条件**的持仓一档（涨幅最大者，不比 `P/V` 边际、不要求弱势）；否则该候选（候选侧 `P/V`）须比最贵的弱势持仓（持仓侧 `P/V`）至少低 `0.16`，且被换出持仓 `收盘 < MA20`；卖一档、买一档。卖出款不定向给触发候选，与其它可用资金一并按 §9.3.2 第 5 步 `P/V` 升序买入 |
...
4. 先卖：
   ...
   - 未持仓合格候选想买但资金不足且满足换仓条件（已持仓候选加仓不触发）：先减持涨幅 ≥110% 的持仓一档（涨幅最大者，不要求弱势），否则减持最贵的弱势持仓一档；同一持仓每日至多换出一档，无其他卖出源即停止换仓。
   - 任何减档后的余仓不足一手时清空。
...
4. 候选臂在剔除集 U 下、标准指标集每一项的配对差均不劣（配对差中位 ≥ −0.15pp；长跑锚点、年均换手与集中度表不计入），记为**去赢家全面优秀**，登记为待考察候选并写入 `docs/Ashare_workflow_open_issues.md`。该判定不构成采纳，也不放宽第 2 款的门槛。采纳前须另行补齐三项：① 换仓边际在该臂上按 0.01 一档重新剂量扫描，不得沿用 `BASE` 的边际；② 剔除赢家只数取 1／3／5／10 的剂量曲线，四档读数须同向；③ 第 10~12 款读数齐备。三项齐备后按第 8 款处理。
...
换估值口径或换宇宙做 A/B 时，**买入线必须重解到同一在册合格面**（`scripts/experimental/align_buy_line.py`），否则比的是两条不同宽度的闸门；**换仓边际不随买入线缩放，按 0.01 一档的剂量扫描重定**。现行两线 **候选侧买入线 1.0454／换仓边际 0.16**：买入线对候选侧状态下侧合格面 17.771%，508,260 个在册观测，保留四位小数、不取整。现行基准（`BASE`：候选侧 `a_share_daily_states_adopted.csv`＋持仓侧 `a_share_daily_states_hold.csv`、授信 66.6%、单票上限 60%、相关性只报告不过滤、涨幅减持 110% 不看走势、T+1 无价跳过、股息税、换仓源同日不重复、同日买卖对冲、配股事件，月末锚定口径）在册读数（全样本）：**滚5 中位／P25／最差 39.18／18.98／0.20、滚5 回撤中位 49.5、滚5 Calmar 0.87、滚5 Sharpe 0.74、负窗口占比 0.5；年化中位 31.26、最大回撤中位 81.2、Calmar 0.39、Sharpe 0.53；5年块中位 49.58；长跑 2009-11 CAGR 25.95／MDD 84.6、2011-11 CAGR 27.15／MDD 86.4；滚3 中位 35.36、滚3 回撤中位 45.3；逐年中位 40.41、逐年最差 −49.5；换手 8.23、仓位 166**。在册读数（去赢家，剔除集 A ＝ 688516／000933／002466／002128／601328）：**滚5 中位／P25／最差 44.97／37.41／15.91、滚5 回撤中位 39.6、滚5 Calmar 1.12、滚5 Sharpe 0.95、负窗口占比 0；年化中位 36.70、最大回撤中位 66.6、Calmar 0.55、Sharpe 0.75；5年块中位 51.67；长跑 2009-11 CAGR 32.11／MDD 65.6、2011-11 CAGR 32.31／MDD 70.3；滚3 中位 44.46、滚3 回撤中位 39.6；逐年中位 37.12、逐年最差 −21.9；换手 4.40、仓位 165**。剔除集 U 随候选臂而变，不入册。配对差一律相对现行基准读数，读数不跨纪元迁移；各纪元的线解与在册读数只查 `docs/Ashare_backtest_log.md`。
