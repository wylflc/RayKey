...
| 持仓只数上限 | 无 |
| 单票机械上限 | 无 |
| 减持 | `P/V ≥ 2.5263` 且 `收盘 < MA20`，减一档 |
| 涨幅减持 | 收盘较持仓均价涨幅 `≥ 125%`（收盘 ≥ 均价 × 2.25）且 `收盘 < MA20`，减一档；持仓均价 = 买入按股数加权、减持不变、除权按 §11.4 折算（持仓表 `cost_basis`） |
| 换仓 | 资金不足时：先换出**满足涨幅减持条件**的持仓一档（涨幅最大者，不比 `P/V` 边际）；否则候选 `P/V` 至少低 `0.1462`，且被换出持仓 `收盘 < MA20`；卖一档、买一档 |
...
**v4.59/v4.60 股本口径纪元**（2026-08-23，OI-086/OI-087，回测日志 §12.118）：逐日状态按每股锚的股本口径全量重建，三线按同一在册合格面重解为 0.9503／2.5238／0.1462；**v4.61 连续判据纪元**（OI-088，回测日志 §12.119）再重解为 **0.9505／2.5263／0.1462**（下侧 17.622%／上侧 30.857%，511,516 观测）（下侧 17.617%／上侧 30.856%，511,416 个在册观测；用户 2026-08-23 裁定采纳），此后配对差一律相对 §12.118 的 NEWL 读数（滚3／滚5／逐年中位 40.70／58.02／43.12、年化中位 41.92、最大回撤中位 66.3、Sharpe 0.69、换手 10.47）。
