...
### OI-156｜待办：MA20 重复减仓保护 X3／X5 的采纳前验证

**来源**：2026-09-06，§12.198、[联合条件续研](reports/whipsaw_joint_review.zh.md)，按工作流 §12.1 第4款登记。`WJ_RF5X3`／`WJ_RF5X5` 的 U=A，标准指标集配对差均未越噪声带；全样本/A Δ年化分别 +0.50/+0.61、+0.00/+0.16pp（联合保护族已试20新臂）。本轮固定完成的是 RF5 的压力与信号审计，不能替代 X3/X5 各自验证。

**待办**：每臂换仓边际按0.01一档重扫；剔除赢家1/3/5/10的剂量曲线；补各自成本压力、信号三表及方向性对照、贡献集中度与臂数登记，再按§12.1处理。先核对触发样本与未来信息边界；中性读数不视为趋势预测能力。当前只登记待考察，不采纳、不改生产。原始配置与证据在 `data/experiments/exp_whipsaw_joint/`。
