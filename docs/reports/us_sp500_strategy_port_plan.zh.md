...
| Yahoo `v8/finance/chart` | 收盘、`adjclose`、拆股与分红事件 | 200 | 免费 | 退市代码 404，覆盖见 §3.3b；作现役代码的价格源 |
...
### 3.3b Yahoo 对历史标普成分的覆盖（2026-09-06，计算节点逐代码探测 829 个）

| 项 | 结果 |
| --- | ---: |
| 现有数据（HTTP 200） | 707 |
| 已无数据（404） | 122，全部是已出指数的代码（344 个出指数区间中的 122 个） |
| 缺失成员·日占比（2011 年起） | 11.2%；2011~2018 年逐年 15%~16%，2020 年 11%，2024 年 3% |
| 代码被新公司复用（有数据但起点晚于入指数日 30 天以上） | 50 个（DOW、CEG、DELL、BBBY、APC 等），不识别就会把别家公司的价格喂进去 |

Yahoo 单源的缺口集中在被收购与破产退市的公司，方向性偏差且早年逼近预登记的 20% 年度阈值，**不能单源使用**。
免费组合：现役且未复用的代码取 Yahoo（无限速、含拆股分红事件）；122 个退市代码与 50 个复用代码（约 170 个）取 Tiingo 免费档（只需注册、不绑卡；50 请求/时、500 代码/月，170 个代码一个作业约 4 小时跑完）。两源都以未复权收盘落同一格式，来源写入价格索引表。

**为什么美股免费源比 A 股麻烦**：美国交易所对行情数据按许可收费、允许再分发的都是付费许可，所以数据商即使有免费档也按请求计量；东财、腾讯这类门户免费转发 A 股行情、且 A 股三十年只退市两百余家，退市历史顺带保留。Yahoo、Stooq 这类免费门户服务现役投资者，公司退市即删数据，而美股每十年有上千家公司因并购退市，「无幸存者偏差」在美国是付费功能。
...
| 融资 | 授信 66.6%、强平 130%、利率 3.5% | 不用融资（`--credit-ratio 0`）；A 股对照臂同样无融资 |
...
**裁定（2026-09-06）**：股票池取 A（标普 500 历史成分）；金融股第一期剔除；美股不用融资、只用现金，A 股对照臂也按无融资跑；其余按推荐。固定取值见预登记书 [us_sp500_backtest_prereg.zh.md](us_sp500_backtest_prereg.zh.md) §0。
