...
## 7. 结果（2026-09-06，依据回测日志 §12.202）

* **样本**：721 个成员区间／673 家非金融公司；CIK 未解析 0.2%、价格缺口 4.2%（无价＋部分覆盖），合计 4.4% 成员·日，最差年 2012 为 12.6%，低于预登记阈值；价格源 Yahoo 525 个代码＋ Tiingo 免费档 158 个代码，仍缺的多为被复用代码（DOW／CEG／FOX／SE／WRK／ADT 等旧主体）与 Tiingo 不返回的退市代码。逐日状态 133 万行，逐年有 `P/V` 的成员·价格日 76%~89%；带级拒绝主因：股数标签缺失 185、正常化 NOPAT 非正 161、NOPAT 不可得 156、母公司权益非正 113、结构断点 211。
* **H1 不支持**：`BASE_US` 滚动 5 年 CAGR 对 `^SP500TR` 同窗口配对差中位 −7.66pp、正号 0/9；全期 −7.77pp、0/9。对齐臂（线 0.6575）更差；滑点每 10bp 约 −0.8pp；预提 0% 与 15% 差在 ±0.5pp 内。
* **并列**：A 股 `BASE` 改无融资后滚 5 中位 24.10（14 起点）／25.22（9 起点），美股 6.81；`P/V` 分档前向回报在美股平坦（Spearman +0.02／+0.04）。
* **成文差异**：持仓侧＝候选侧（未移植 B2）；分拆未建模；控股公司重组按指数退出／进入拆分；退市名册含闭段文件末端。
* 产物：`data/experiments/exp_us_sp500_port/`（扫描、H1 读数、分档表、覆盖率），作业 `scripts/slurm/us_sp500_*.sbatch`。
* **追加（2026-09-07，§12.203）**：股票池缩小为标普 100 历史成分（134 家非金融）后滚 5 配对差 −4.26pp、全期 −3.86pp，仍 0/9 为正，但差距较标普 500 池收窄一半；全期年化 10.52%、最大回撤 33.1%、换手 2.09。
